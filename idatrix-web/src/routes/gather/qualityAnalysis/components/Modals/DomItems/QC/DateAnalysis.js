...
import { Form,Select,Checkbox,Input,Row,Col ,Tooltip} from 'antd';
            ...
            <Form >
                ...
                )}
                {
                    ...
                    >
                        ...
                        data.dictionaryList.allValue : 
                        (referenceValues.length > 0 && !this.state.covered ? referenceValues : []),
                        rules: [
                            {required: true, message: "请选择参考值" }
                        ]
                        })(
                            <CheckboxGroup style={{border:"1px solid #f0f0f0",borderRadius:"8px", padding: "16px"}} onChange={this.checkSelected}>
                                <Row>
                                    {  data.dictionaryList.allValue.map((val,index)=>(
                                        <Col span={12} key={index + val}>
                                            <Checkbox value={val}>
...
const DateAnalysisForm = Form.create()(PluginComponent(DateAnalysis,{width:960,dictionaryId: 3}));

export default connect()(DateAnalysisForm);
