...
import './index.less'
import { getDb_list2,getHadoop_list,get_db_table,get_db_table_fields,getDefaultEngineList,getDataStore,getTrans_list,get_SftpList,trans_VariablesList,
         get_FileExist,get_output_fields,check_step_name,save_step,get_input_fields,get_details,get_db_schema,getServer_list,getDictionary
...
let Timer;
const DomShow = ({domItems,dispatch,cloudetlCommon})=>{
  ...
  const getInputSelect = (data,type,symbol)=>{
    ...
    for(let index of data){
      args.push(
        <Select.Option key={type?index[type]:index} value={sign+(type?index[type]:index)}>{sign+(type?index[type]:index)}</Select.Option>
      )
    }
    return args;
  };
...
//获得下拉框 Multiway Merge Join组件专用
  const getInputSelectMultiway = (symbol,symbol1,data,type,symbol2)=>{
    ...
    for(let index of data){
      args.push(
        <Select.Option key={type?index[type]:index} value={sign+sign1+(type?index[type]:index)+sign2}>{sign+sign1+(type?index[type]:index)+sign2}</Select.Option>
      )
    }
    return args;
  };
  ...
  const get_Similarity = (args1,args2,source,target,str)=>{
    ...
    let newArgs = args1.map(item=>{  
        for(let index of args2){
            if(index[field].toUpperCase() === item[source].toUpperCase() || 
              index[field].toUpperCase().indexOf(item[source].toUpperCase()) !== -1 || 
  ...
  switch (items.panel){
    ...
    case "Redundance": return ( <Redundance model={items} />); 
...
export default connect(({domItems,cloudetlCommon})=>({
    domItems,cloudetlCommon
}))(DomShow)
