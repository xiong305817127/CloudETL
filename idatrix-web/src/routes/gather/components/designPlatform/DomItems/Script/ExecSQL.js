...
class Exec extends React.Component{
  ...
  Request(){
    const { getInputFields,transname,text,selectOption,getInputSelect } = this.props.model;
    let obj = {};
  ...
  render(){
    ...
    return(
      <Modal
        ...
        maskClosable={false}
        footer={[
                  <Button key="submit" type="primary" size="large"  onClick={this.handleFormSubmit.bind(this)}>
                    确定
                  </Button>,
      ...
      >
        <Form >
          ...
          <FormItem {...this.formItemLayout2} label="要执行的SQL脚本（用 ; 号分隔语句，问号将被参数替换）" style={{marginBottom:"8px",display:"none",marginLeft:"10%"}} >
            ...
            <Row style={{margin:"5px 0",width:"100%"}}  >
              <Col span={12}>
                <ButtonGroup  size={"small"} >
                  <Button  disabled={!setDisabled()}  onClick={this.handleAdd.bind(this)}>添加字段</Button>
                  <Button  disabled={!setDisabled()}  onClick={this.handleDeleteFields.bind(this)}>删除字段</Button>
                </ButtonGroup>
              </Col>
              <Col span={12} style={{textAlign:"right"}}>
                <Button  disabled={!setDisabled()} size={"small"}  onClick={this.getInputFields.bind(this)}>获取字段</Button>
              </Col>
            </Row>
            <EditTable  disabled={!setDisabled()} columns={this.columns} dataSource = {this.state.input_fields} rowSelection={true} tableStyle="editTableStyle5" size={"small"} scroll={{y: 140}} ref="editTable"   count={4}/>
          </div>

          <FormItem label="包含插入状态的字段"  {...this.formItemLayout6}  style={{marginBottom:"8px"}}>
          ...
          <FormItem label="包含更新状态的字段"  {...this.formItemLayout6}  style={{marginBottom:"8px"}}>
          ...
          <FormItem label="包含读状态的字段"  {...this.formItemLayout6}  style={{marginBottom:"8px"}}>
          ...
          <FormItem label="包含删除状态的字段"  {...this.formItemLayout6}  style={{marginBottom:"8px"}}>
...
export default connect()(ExecSQL);
