...
class SetValueDialog extends React.Component {
    ...
    form.validateFields((err, values) => {
      ...
      if(this.refs.editTable){
        if(this.refs.editTable.state.dataSource.length>0){
          let args = [ "fieldName", "replaceByFieldValue"];
  ...
  columns =  [{
    ...
    title: '替换字段值',
    dataIndex: 'replaceByFieldValue',
    width:"46%",
    key: 'replaceByFieldValue',
    selectable:true
  }];
...
const SetValueField = Form.create()(SetValueDialog);

export default connect()(SetValueField);
