...
class HadoopInputDialog extends React.Component {
    ...
    form.validateFields((err, values) => {
      ...
      if(this.refs.editTable){
        if(this.refs.editTable.state.dataSource.length>0){
          let args = [ "sourceConfigurationName", "fileName", "fileMask", "excludeFileMask", "fileRequired", "includeSubFolders"];
  ...
  handleGetOutField(){
    ...
    form.validateFields((err,values)=>{
      ...
      obj.detailParam = {
        flag:"getFields",
        fileName:"hdfs::"+sendFields[0].sourceConfigurationName+"::"+sendFields[0].fileName,
        content:obj1
      };
  ...
  getFieldList(name){
    ...
    let str = getFieldValue("file");

    if(str && str.trim()){
      let obj = treeViewConfig.get(panel)[name];
      let path = formatFolder(getFieldValue("files"))?formatFolder(getFieldValue("files")):"/";
          ...
          <div style={{margin:"0 5%"}}>
            <Tabs type="card"   >
              <TabPane tab="文件" key="1">
               ...
               <FormItem label="Hadoop环境"  {...this.formItemLayout1} style={{marginBottom:"8px"}}>
                  {getFieldDecorator('file', {
                    initialValue: ""
                  })(
                      <Select onFocus={this.Request.bind(this)} disabled={setDisabled()} >
                ...
                <FormItem label="源文件"  {...this.formItemLayout1} style={{marginBottom:"8px"}}>
                  {getFieldDecorator('files', {
                    initialValue: "",
                  })(
                       <Input  disabled={setDisabled()}/>
                  )}
                   <Button onClick={()=>{this.getFieldList("model")}} disabled={setDisabled()}>浏览</Button>
                </FormItem>
                  ...
                  <Row style={{marginBottom:"5px"}}>
                    ...
                    <Col span={12}>
                      <ButtonGroup size={"small"} style={{float:"right"}} >
                        <Button  onClick={this.handleAdd.bind(this)} disabled={setDisabled()}>添加文件</Button>
                        <Button  onClick={this.handleDeleteFields.bind(this)}  disabled={setDisabled()}>删除文件</Button>
                      </ButtonGroup>
                    </Col>
                  </Row>
                </div>
                <EditTable disabled={setDisabled()}  columns={this.fileColumns} tableStyle="editTableStyle5" ref="editTable" scroll={{y: 140}} rowSelection={true}  size={"small"} count={1}  dataSource={this.state.fileSource }/>
              ...
              <TabPane tab="内容" key="2">
                    ...
                    <FormItem label="压缩"   style={{marginBottom:"0px"}} {...this.formItemLayout4}>
                      {getFieldDecorator('fileCompression', {
                        ...
                        <Select>
                          ...
                          <Option disabled value="Hadoop-snappy">Hadoop-snappy</Option>
                ...
                <Row  style={{marginLeft:"10%",marginBottom:"8px"}}>
                  <Col span={4}>
                    <FormItem  style={{marginBottom:"0px",marginLeft:"-25px"}} {...this.formItemLayout}>
                  ...
                  <Col span={18}>
                    <FormItem  label="包含文件名的字段名称" style={{marginBottom:"8px"}} {...this.formItemLayout7}>
              ...
              <TabPane tab="过滤" key="4">
                ...
                <EditTable  columns={this.filterColumns}   tableStyle="editTableStyle5" ref="editTable1" scroll={{y: 300}} rowSelection={true}  size={"small"} count={1}  dataSource = {this.state.filterSource}/>
...
export default connect()(HadoopFileInputPlugin);
