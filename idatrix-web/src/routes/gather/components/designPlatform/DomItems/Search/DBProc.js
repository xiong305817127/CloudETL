...
import { Form, Select, Button, Input, Checkbox, Icon, Row, Col, message } from 'antd';
...
class DBProcInput extends React.Component {
  ...
  columns = [
    {
      ...
      width:"30%",
      selectable:true,
      selectArgs:[<Select.Option key="IN" value="IN" >IN</Select.Option>,
        <Select.Option key="OUT" value="OUT" >OUT</Select.Option>,
        <Select.Option key="INOUT" value="INOUT" >INOUT</Select.Option>
      ]
    }, {
      title: '类型',
      dataIndex: 'type',
  ...
  //4.4：选中SQl
  getSchemaList = async (a)=>{
    // console.log(a,111);
    if(a){
      const { transname } = this.props.model;
      console.log(transname,111);
      let obj = {};
      obj.connection = a;
      const data = await get_ProcList(obj);
         console.log(obj,data,222);
      if (data && data.data){
        this.setState({db_model: data.data.data});
        // console.log(this.state.db_model,3333);
      }
  ...
  render() {
    ...
    console.log(this.state.dbList,"dbList");
    const setDisabled = ()=>{
      if(getFieldValue("autoCommit") === undefined){
        return config.autoCommit;
      }else{
        if(getFieldValue("autoCommit")){
          return getFieldValue("autoCommit");//true
        }else {
          return false;
        }
      }
          ...
          <div style={{border:"1px solid #D9D9D9"}}>
            <fieldset className="ui-fieldset">
              ...
              <Form >
                ...
                <FormItem label="储存过程名称"   {...this.formItemLayout2} style={{marginBottom:8}}>
                  {getFieldDecorator('procedure', {
                    ...
                    >
                      {
                        this.state.db_model.length>0?this.state.db_model.map((index)=>
                ...
                <FormItem label="启用自动提交"   {...this.formItemLayout2} style={{marginBottom:8}}>
                ...
                <FormItem label="返回值名称"   {...this.formItemLayout2} style={{marginBottom:8}}>
                ...
                <FormItem label="返回值类型：" {...this.formItemLayout2} style={{marginBottom:8}}>
                  {getFieldDecorator('resultType', {
                    ...
                    >
                      ...
                      <Select.Option value="Internet Address">Internet Address</Select.Option>
                    </Select>
                  )}
                </FormItem>
              </Form>
