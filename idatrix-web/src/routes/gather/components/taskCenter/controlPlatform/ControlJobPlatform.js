...
import * as gatherFuncs from "../../../../../services/gather1";
...
class ControJobPlatform extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleCancel() {
    ...
    dispatch({
      type: "controljobplatform/hideModel",
      visible: false
    });
  }
  ...
  dynamicRender(obj) {
    ...
    if (errorData) {
      for (let index of errorData) {
        const { logText, errCount, entryName } = index;
  ...
  dynamicInfo(statusData, bool) {
    ...
    let newStatusData = statusData[0] ? statusData[0].childEntryMeasure : [];
    console.log(statusData, "步骤信息状态！");

    const series = this.state.option.series;
    const data = series[2].data;
    for (let index of newStatusData) {
      const { result, reason, entryName, comment } = index;
      for (let item of data) {
        if (entryName === item.name) {
          item.result = result;
          item.reason = reason;
          item.statusDescription = comment;
        }
      }
    }
  ...
  sendTransEvent({ executionId }) {
    const { getJob_exec_step_measure } = this.props.jobheader.methods;
    getJob_exec_step_measure(executionId).then(res => {
  ...
  autoRunView(transName) {
    ...
    const { getJob_exec_id } = this.props.jobheader.methods;
    getJob_exec_id({ name: transName }).then(res => {
      const { code, data } = res.data;
      if (code === "200") {
  ...
  componentWillReceiveProps(nextProps) {
    const {
      ...
      transName,
      owner
    } = nextProps.controljobplatform;
    const { getJob_status } =
      owner == "" ? this.props.jobheader.methods : gatherFuncs;

    if (actionType === "openView") {
      getJob_status({ name: transName }).then(res => {
        const { code, data } = res.data;
        const { status } = data;
  ...
  timeTest(func, w, obj) {
    const {
      ...
      getJob_exec_step_measure
    } = this.props.jobheader.method;
    var interv = () => {
      getJob_status({ name: obj.name }).then(res => {
        ...
        if (code === "200" && runStatus.has(status)) {
          ...
          getJob_exec_step_status({ executionId: obj.executionId }).then(
            res => {
              const { code, data } = res.data;
  ...
  handleRunTrans() {
    const { transName } = this.props.controljobplatform;
    const { dispatch } = this.props;
    dispatch({
  ...
  handleEndTrans() {
    ...
    const { transName } = this.props.controljobplatform;
    dispatch({
      type: "controljobplatform/stopJob",
      payload: {
        actionName: transName
      }
    });
  ...
  render() {
    const { visible, transName } = this.props.controljobplatform;
...
export default connect(({ controljobplatform, jobheader }) => ({
  controljobplatform,
  jobheader
}))(ControJobPlatform);
