...
import { Button,Radio,Tabs,Checkbox,Table,Form,Input,Cascader,Row,Col,message,Modal,Select} from 'antd';
import Style from './DataSystemModel.css';
import EditTable from '../../common/EditTable'
import Empower from '../../../../../components/Empower';
import { withRouter } from 'react-router';
import { save_db_connection,check_dbname_result } from '../../../../../services/gather';
...
class DataDetail extends React.Component{
  ...
  getModel(config){
    ...
    switch (this.getType(value)) {
      case 11:
        return (
          <div>
            ...
            <FormItem label="用户名"   style={{marginBottom:"8px"}} {...this.formItemLayout2} >
            ...
            <FormItem label="密码"   style={{marginBottom:"8px"}} {...this.formItemLayout2} >
      ...
      case 12:
        return (
          <div>
            ...
            <FormItem style={{margin:"0px",marginLeft:"16%"}} {...this.formItemLayout1}>
      ...
      case 13:
        return (
          <div>
            ...
            <FormItem label="数据表空间"  style={{marginBottom:"8px"}} {...this.formItemLayout2} >
            ...
            <FormItem label="索引表空间"   style={{marginBottom:"8px"}} {...this.formItemLayout2}>
            ...
            <FormItem label="端口号"   style={{marginBottom:"8px"}} {...this.formItemLayout2} >
      ...
      case 14:
        return (
          <div>
            ...
            <FormItem label="示例名称"   style={{marginBottom:"8px"}} {...this.formItemLayout2}>
      ...
      case 15:
        return(
          <div>
            <FormItem label="自定义连接 URL"  style={{marginBottom:"8px"}} {...this.formItemLayout2} >
            ...
            <FormItem label="自定义驱动类名称"   style={{marginBottom:"8px"}} {...this.formItemLayout2}>
            ...
            <FormItem label="命名空间"  style={{marginBottom:"8px"}} {...this.formItemLayout2} >
                     ...
                     <Select>
                       {
                         JNDIargs.map(index=>{
                           return(
                             <SelectOption key={index.key} value={index.name}>{index.name}</SelectOption>
                           )
                         })
                       }
                     </Select>
          ...
          <div>
            ...
            <FormItem style={{margin:"0px",marginLeft:"16%"}} {...this.formItemLayout1}>
              {getFieldDecorator('modifier32', {
                ...
                <Checkbox value="public" disabled>Use Result Streaming Cursor</Checkbox>
              )}
            </FormItem>
          </div>
        );
          ...
          <div>
            <FormItem label="JNDI源名称"    {...this.formItemLayout1} >
              <Row gutter={8}>
                ...
                <Col span={10}>
                  {
                    this.jndiDataSource(33)
                  }
                </Col>
              </Row>
            </FormItem>
            <FormItem label="数据表空间"  style={{marginBottom:"9px"}} {...this.formItemLayout2} >
          ...
          <div>
            <FormItem label="JNDI源名称"    {...this.formItemLayout1} >
              <Row gutter={8}>
                 ...
                 <Col span={10}>
                   {
                     this.jndiDataSource(34)
                   }
                 </Col>
              </Row>
            </FormItem>
            <FormItem style={{margin:"0px",marginLeft:"16%"}} {...this.formItemLayout2}>
  ...
  handleCreate (e) {
    ...
    form.validateFields((err, values) => {
      ...
      obj.dataTableSpace = values["dataTableSpace"+this.getType(values.residence)];
      obj.indexTableSpace = values["indexTableSpace"+this.getType(values.residence)];
      obj.sqlServerInstance = values["sqlServerInstance"+this.getType(values.residence)];
      obj.useIntegratedSecurity = values["useIntegratedSecurity"+this.getType(values.residence)];
      obj.useDoubleDecimalSeparator = values["useDoubleDecimalSeparator"+this.getType(values.residence)];
      obj.databaseName = values["databaseName"+this.getType(values.residence)];
      obj.username = values["username"+this.getType(values.residence)];

      if(values["password"+this.getType(values.residence)] && values["password"+this.getType(values.residence)].length>0){
        obj.password =  strEnc(values["password"+this.getType(values.residence)],values.name,values["hostname"+this.getType(values.residence)],values["port"+this.getType(values.residence)]);
      }
      ...
      if(this.refs.editTable){
        if(this.refs.editTable.state.dataSource.length>0){
          ...
          sendFields = this.formatTable(this.refs.editTable.state.dataSource,args);
        }
      }else{
        if(options){
          sendFields = options
  ...
  jndiDataSource(num){
    return(
          <ButtonGroup>
            ...
            <Button onClick={()=>{this.handleEditJNDI(num)}}>编辑</Button>
          </ButtonGroup>
    )
  };
    ...
    return(
      <div id="data_detail" >
        <Row  className={Style.ColList}  >
          <Col span={2} className={this.state.activeKey === "1"?Style.click:""}  onClick={()=>{this.handleTabsClick("1")}}> 一般</Col>
          <Col span={2} className={this.state.activeKey === "2"?Style.click:""} onClick={()=>{this.handleTabsClick("2")}}>高级</Col>
          <Col span={2} className={this.state.activeKey === "3"?Style.click:""} onClick={()=>{this.handleTabsClick("3")}}>选项</Col>
          <Col span={2} className={this.state.activeKey === "4"?Style.click:""} onClick={()=>{this.handleTabsClick("4")}}>连接池</Col>
          <Col span={2} className={this.state.activeKey === "5"?Style.click:""} onClick={()=>{this.handleTabsClick("5")}}>集群</Col>
        </Row>

        <Form  id="datasystem_form" >
          <Tabs  type="card"  activeKey={this.state.activeKey}>
            <TabPane tab="一般" key="1">
              <FormItem label="连接名称" style={{marginTop:"20px"}}   {...this.formItemLayout2}>
                {getFieldDecorator('name', {
                  initialValue:config.name,
                  rules: [{ required: true, message: '请输入连接名称' },
                    {validator:this.handleConfirm,message: '连接名称已存在，请更改!' }
                  ]})(
                  <Input  disabled={config.name?true:false}/>
                )}
              </FormItem>
              <FormItem
                label="连接类型/方式"  {...this.formItemLayout2}
              >
                {getFieldDecorator('residence', {
                  initialValue: this.state.value
                })(
                  <Cascader options={this.residences} onChange={(value)=>this.cascaderChange(value)} />
                )}
              </FormItem>
              {
            ...
            <TabPane tab="高级" key="2">
              <FormItem {...this.formItemLayout} style={{marginBottom:"0px",marginTop:"20px",marginLeft:"50px"}} >
              ...
              <FormItem {...this.formItemLayout} style={{marginBottom:"0px",marginLeft:"50px"}} >
              ...
              <FormItem label="默认名称(在没有其他模式名时使用)"  {...this.formItemLayout3} style={{marginBottom:"0px",marginLeft:"50px"}} >
              ...
              <FormItem label="请输入连接成功后要执行的SQL语句,用分号(;)隔开" {...this.formItemLayout} style={{marginBottom:"8px",marginLeft:"50px"}} >
            ...
            <TabPane tab="选项" key="3">
              <div style={{padding:"20px"}}>
                <EditTable extendDisabled={true} columns={this.columns} dataSource = {this.dataSource} size={"small"}  ref="editTable"  count={6}/>
              </div>
            </TabPane>
            <TabPane tab="连接池" key="4">
              <div style={{padding:"20px"}}>
                ...
                <EditTable  extendDisabled={true} size={"small"} disabled={setDisabled()} tableStyle="editTableStyle3" columns={this.columns1}  dataSource={this.dataSource1}  pagination={false} />
                <EditTable extendDisabled={true} rowSelection={true} size={"small"} disabled={setDisabled()} tableStyle="editTableStyle2" columns={this.columns2}  dataSource={this.dataSource2} scroll={{ y: 260 }} pagination={false} />
                <Button size={"small"} disabled={setDisabled()} style={{marginTop:"10px",marginRight:"5px",float:"right"}}>恢复默认设置</Button>
            ...
            <TabPane tab="集群" key="5">
              <div style={{padding:"20px"}}>
                ...
                <div style={{marginTop:"20px",marginBottom:"10px",marginLeft:"5px"}}>命名参数：</div>
                <EditTable extendDisabled={true} disabled={setDisabled1()} size={"small"} columns={this.columns3}  dataSource={this.dataSource3}  />
              </div>
            </TabPane>
          </Tabs>
          <Row className={Style.BottomRow}>
            <Col span={12}  style={{textAlign:"right"}}>
              <Empower api={canEdit?"/db/editDbConnection.do":""} >
                <Button onClick={this.handleCreate.bind(this)} type="primary" htmlType="submit" >
                  保存
                </Button>
              </Empower>
            </Col>
...
export default withRouter(connect(({ resourcecontent,transheader }) => ({
  resourcecontent, transheader
}))(DataSystemModel));
