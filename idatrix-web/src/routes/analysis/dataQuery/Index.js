...
class DataQueryTable extends React.Component {
  ...
  onSelect = async (selectedKeys, e) => {
    // console.log('111:',selectedKeys);
     console.log(selectedKeys,'点击菜单列表:',e.node.props);
    ...
    // 查询数据库相关信息，并更新自动完成提示
    const ds = e.node.props.titleDefault;
   if(e.node.props.pos !== "0-0-0" && e.node.props.pos !== "0-0-1" &&
      ...
      if (ds) {
        const { data } = await getSchemaInfo({ ds });
        const info = data && data.code ==="200" || {};
        const schemaInfo = createSchemaInfo(info, ds);
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err,values) =>{
      if(!err){
        ...
        obj.prepSchema = this.state.prepSchema; //获取schemaname
    ...
    return(
      <Layout  style={{minheight:'100%'}}>
        ...
        <Sider style={{backgroundColor:'#ffffff',overflow:'auto'}} >
          ...
          >
            <TreeNode key="数据系统" title="数据系统" titleDefault="splDefault">
              {
                Object.keys(dbList).map(key => {
                  return (
                    <TreeNode key={key} title={key}  headMenu={key} titleDefault="splDefault" >
                      {
                        dbList[key].map((keyChild, index)=> {
                          return (
                            <TreeNode key={keyChild.dbname} title={keyChild.dbname} actionValue={keyChild.dsname} headMenu={index} titleDefault={keyChild.dsname}>
                              {
                                keyChild.tableList.map((tableName, item)=> {
                                  //titleDefault是传的值
                                  return (
                                    <TreeNode key={tableName.tableName} title={tableName.tableName} actionValue={tableName.schemaName} headMenu={key} titleDefault={keyChild.dsname}></TreeNode>
                                  );
                                })
                              }
                            </TreeNode>
