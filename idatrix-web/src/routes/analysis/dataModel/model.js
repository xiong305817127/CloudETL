import { getModelList, addNewFolder, isExistFolder, isExistName, deleteModel, deleteFolder, importToSaiku,getRentTablesBySchemaId } from "services/bi";
import { getDatabaseListByRentId } from 'services/catalog';
...
const initState = {
	//数据源数组
	options: [{label:"MySql",value:3,isLeaf: false}],
	//目录树数组
	folderTreeList: [],
	//新建文件夹
	newFolder: false,
...
export default {
	...
	effects: {
		...
		*getDataSource({ payload, resolve, reject }, { select, put, call }) {
			...
			if(payload.length && payload.length === 2 ){
				data = yield call(getRentTablesBySchemaId, { dsId: payload.dsId, rentId: renterId });
			}else{
				data = yield call(getDatabaseListByRentId, { dsType: payload.dsType, rentId: renterId });
			}
			const { code } = data.data;
			if (code === "200") {
				payload.targetOption.loading = false;
				if(payload.length === 2){
					payload.targetOption.children = data.data.data.tableList.map(index => {
						return {
							label: index.tableName,
							value: index.id,
							disabled:true
						}
					});
	...
	reducers: {
		...
		hide(state, action) {
			...
			return { ...state, ...action.payload,options:[{label:"MySql",value:3,isLeaf: false}] }
		},
		clear(state, action){
			...
			return { ...initState,options:[{label:"MySql",value:3,isLeaf: false}]}
		}
	}
}