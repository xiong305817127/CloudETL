...
class index extends React.Component {
	...
	//开始拖拽，赋初始值
	drag = (e, item) => {
		e.dataTransfer.setData("id", item.metaid + "");
		e.dataTransfer.setData("tableName", item.metaNameEn);
	}
		...
		dispatch({
			...
			payload: {
				tables: tables.map(index => {
					if (index.metaNameEn.indexOf(value) !== -1) {
				...
				<div className={styles.silder}>
					...
					<div className={styles.silderList}>
						<List
							...
							renderItem={item => (
								<List.Item draggable="true" key={item.metaid} onDragStart={(e) => { this.drag(e, item) }} className="item" >
									<img src={require("assets/images/analysis/table.png")} />
									<span className="text">{item.metaNameEn}</span>
