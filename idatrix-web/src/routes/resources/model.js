...
import { getAllNode,getAllOrgs,getAllServices,getRoles,getSubtreeAndDepth } from 'services/catalog';
...
export default {
  ...
  effects: {
    ...
    *getAllResource({ force }, { put, select }) {
      const { account, resourcesCommon } = yield select(state => state);
      const { industryOptions, themeOptions, tagsOptions } = resourcesCommon;
