...
const index = ({form,dispatch,RelationshipModel,location,router})=>{

  const {getFieldDecorator} = form;
  const {total,datasource,loading,selectedRows} = RelationshipModel;
  ...
  const handleCheckView = (record)=>{
   hashHistory.push(`/resources/subscription/Relationship/datali/${record.id}`)
  }
  //恢复订阅
  const handleOprateryes=(record)=>{
     dispatch({type:"RelationshipModel/getresume",payload:{id:record.id}})
  }
  //终止订阅
  const handleOpraterNo=(record)=>{
    dispatch({type:"RelationshipModel/getstop",payload:{id:record.id}})
  }
...
export default connect(({ RelationshipModel })=>({ RelationshipModel }))(withRouter(Form.create()(index)));