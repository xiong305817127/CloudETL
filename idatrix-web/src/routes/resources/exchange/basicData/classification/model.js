import { addClassify, getDict, deletelist, forceDelete } from 'services/DirectoryOverview';
...
export default {
	...
	subscriptions: {
		setup({ history, dispatch }) {
			return history.listen(({ pathname, query }) => {
				if (pathname === "/resources/exchange/basicData/classification") {
	...
	effects: {
		...
		*getList({ payload }, { call, select, put }) {
			const { data } = yield call(getDict, { type: "classify" });
			const { code } = data;
			if (code === "200") {
				let dataId=[];
		...
		*getSobmit({ payload }, { call, select, put }) {
			...
			if (code === "200") {
				message.info("保存成功");
				yield put({ type: "getList", payload:{ type: "classify",status:4 } })

			}else{
				yield put({ type: "save", payload:{ status:6 } })
		...
		*getDeletelist({ payload }, { call, select, put }) {
			...
			if (code === "200") {
				...
				yield put({ type: "getList", payload: { type: "classify" } })
			}
			//forceDelete
			//if(){}
