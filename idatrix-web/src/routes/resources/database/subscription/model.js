import { getManage,stop,resume } from 'services/DirectoryOverview';
...
export default {
	...
	subscriptions:{
		setup({history,dispatch}){
	      	return history.listen(({ pathname,query })=>{
		        if(pathname === "/resources/database/subscription"){
		            dispatch({
	...
	effects:{
		*getList({payload},{put,select,call}){
			...
			const { data } = yield call(getManage,{...payload});
			const { code } = data;
			if(code === "200"){
		        yield put({
		            type:'save',
