   ...
   const handleChange = (ids,options)=>{
    	...
    	if(filesId === options){
          message.error("文件类资源目录与上报文件目录不能一致");
	    	...
	    	dispatch({type:"systemparmModel/save",payload:{fileRoot:args.join("/"),fileId:options}})
    	}
    }
   ...
   const handleChange1 = (ids,options)=>{
    	if(fileId === options){
          message.error("文件类资源目录与上报文件目录不能一致");
    		...
    		let args = [];
		    	...
		    	dispatch({type:"systemparmModel/save",payload:{originFileRoot:args.join("/"),filesId:options}})
    	}
    ...
    const handleSubmit = (e)=>{
	    ...
	    if (!err) {
	        ...
	        dispatch({type:"systemparmModel/handleSubmit",payload:{...values,
	        	id:id?id:0,
	        	fileRoot:"/"+fileRoot+"/",originFileRoot:"/"+originFileRoot+"/",
	        	fileRootIds:values.fileRootIds.join(","),
	        	originFileRootIds:values.originFileRootIds.join(","),
    ...
    const handleCheckName = (rule,value,callback)=>{
    	...
    	let newValue = getFieldValue("fileRootIds").join("/");

    	console.log(newValue);
    	console.log(value.join("/"));

    	if(value && value.join("/") === newValue){
    		callback(true);
    	}else{
    		callback();
    	}
			...
			<Form style={{maxWidth:"600px"}} >
				<FormItem label="文件类资源目录" {...formItemLayout}>
		          {getFieldDecorator("fileRootIds", {
		          	initialValue:config.fileRootIds?config.fileRootIds:[],
		         ...
		         {getFieldDecorator("originFileRootIds", {
		         	initialValue:config.originFileRootIds?config.originFileRootIds:[],
		            rules: [{required: true, message: '请输入上报文件目录！'},
		            	{validator:handleCheckName,message:"上报文件目录不能与文件类资源目录重复！"}
		            ],
		          })(
		             <Cascader
			           	 placeholder="请选择上报文件目录"		
			              options={hdfsTree}
		         ...
		         {getFieldDecorator("importInterval", {
		            ...
		            rules: [
		            {required: true, message: '请输入导入扫描时间间隔！'}]
		          })(
		          ...
		          {getFieldDecorator("deptAdminRole", {
		            ...
		            rules: [{required: true, message: '请输入部门管理员！',}],
		          	...
		          	<Select placeholder="请选择部门管理员"	>
						{
							rolesList.map(index=>(<Option key={index.id} value={index.id+""}>{index.name}</Option>))
						}
		          	</Select>
		          )}
		        </FormItem>
		        <FormItem label="数据中心管理员" {...formItemLayout}>
		          {getFieldDecorator("centerAdminRole", {
		          	initialValue:config.centerAdminRole?config.centerAdminRole+"":"",
		        ...
		        <FormItem label="订阅审批角色" {...formItemLayout}>
		          {getFieldDecorator("subApproverRole", {
		          	initialValue:config.subApproverRole?config.subApproverRole+"":"",
...
export default connect(({ systemparmModel,resourcesCommon })=>({ 
	systemparmModel,resourcesCommon }))(Form.create()(index));