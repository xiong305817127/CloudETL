import { getSubtreeAndDepth,getNodeInfo,addNode,deleteNode,fileImport } from 'services/catalog';
  ...
  effects: {
    ...
    *getResourcesFolder({ payload,resolve,treeNode,resourcesList }, { put,call,select }) {
			...
			if(code === "200"){
				if(resourcesList && resourcesList.length > 0){
					...
					yield put({ type: 'save', payload: { resourcesList:databaseModel.resourcesList } });
				}				
			}
		},
		*updateResourcesFolder({ payload },{ call,select,put }){
			...
			if(code === "200"){
				...
				//递归更新resourceList 
				let newResourceList = getNewResouceList(resourcesList,payload.id,newData);
				yield put({ type:'save',payload:{ resourcesList:newResourceList }})
			}
		},
    *deleteNodeInfo({ payload }, { put,call,select }) {
      ...
      if(code === "200"){
        ...
        yield put({type:"updateResourcesFolder",payload:{ id:config.parentId }});
      }
    },
    ...
    *saveNode({actionType,payload},{put,call}){
        ...
        if(code === "200"){
           if(actionType === "edit"){
            yield put({ type:"save",payload:{visible:false,config:{...payload}}})
          }else{
            yield put({ type:"save",payload:{visible:false}})
          }
          yield put({type:"updateResourcesFolder",payload:{ id:payload.parentId } })
        }
    },
    *fileImport({payload},{put,call}){
        ...
        if(code === "200"){
          ...
          yield put({type:"updateResourcesFolder",payload:{ id:config.id }})
        }else{
          hide();
