import { getSubtreeAndDepth,getFieldList, getResource, getTableList, saveInfo, getProcess, getDatabaseListByRentId } from 'services/catalog';
    ...
    *getResourcesFolder({ payload,resolve,targetOption,resourcesList, }, { put,call,select }) {
			...
			if(code === "200"){
				if(resourcesList && resourcesList.length > 0){
					...
					yield put({ type: 'save', payload: { resourcesList:sourceEditView.resourcesList } });
				}				
			}
		},
		*getDatabase({ payload, force }, { call, select, put }) {
			...
			const { data } = yield call(getDatabaseListByRentId, { dsType: arrType, rentId: account.renterId })
			const { code } = data;
			...
			if (code === "200") {
				...
				for (let index of data.data) {
					if (!unqieArgs.includes(index.name)) {
						...
						args.push({ value: index.name, label: index.name, dsId: index.dsId, isLeaf: false })
					}
				}
				yield put({ type: "save", payload: { bindTables: args, updataType: "database" } });
			}
		},
		*getTable({ payload }, { call, select, put }) {
			...
			if (code === "200") {
				...
				if (schemaIndex >= 0) {  //schemaIndex如果为0 会不往下面流程走，不会取最后一层
		...
		*getFields({ payload }, { call, select, put }) {
			const { account } = yield select(state => state);
			const { data } = yield call(getFieldList, { metaId: payload.tableName, userId: account.username });
			...
			if (code === "200") {
				...
				yield put({ type: "save", payload: { fieldsList, updataType: "field" } })
			}
		},
		*getEditResource({ payload }, { call, select, put }) {
			...
			if (code === "200") {
				...
				if (config.libTableId) {
					...
					yield put({ type: "getTable", payload: { dsType: config.formatInfo, bindTableId: tempIds, name: tempIds[0] } })
					yield put({ type: "getFields", payload: { userId: account, tableName: tempIds[2] } })
				}
				...
				yield put({
					type: "save",
					payload: {
						config, updataType: "config", controlVisible, open, shareType: config.shareType,//保存提供方代码
						deptCode: config.deptCode, deptName: config.deptName, catalogCode, dataSource, shoudleUpdate: true
					}
				})
			}
		},
		*saveFormInfo({ payload, str }, { call, select, put }) {
			...
			if (code === "200") {
				if (str && str === "save") {
					...
					yield put({ type: "save", payload: { formChange: false, dataSource: payload.resourceColumnVOList } });
				} else {
