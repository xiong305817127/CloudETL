...
class index extends React.Component {
	...
	handleFormatTypeChange = (e, options) => {
		...
		if (options[1]) {
			...
			dispatch({ type: "sourceEditView/save", payload: { name: e[0], tableName: code } });
		}
		...
		if (selectedOptions.length === 2) {
			...
			getRentTablesBySchemaId({
				...
				if (code === "200") {
					...
					const secondIndex = newBindTables[firstIndex].children.findIndex(val => {
						return val.label === selectedOptions[1].label
					});

					if (data.tables.length !== 0) {
						newBindTables[firstIndex].children[secondIndex].loading = false;
						newBindTables[firstIndex].children[secondIndex].children = data.tables.map((val) => {
							return {
								value: val.id,
								label: val.tableName
							}
						...
						newBindTables[firstIndex].children[secondIndex].children = [{ value: "暂无数据表", label: "暂无数据表", disabled: true }];
	...
	handleResourceChange = (e, options) => {
		...
		if (options[2]) {
			...
			dispatch({ type: "sourceEditView/getFields", payload: { name: e[0], tableName: value } });
		}/**
      else{
       dispatch({ type: "sourceEditView/getFields", payload: { name: e[0], tableName: value } });
    }
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				...
				if (values.bindTableId) {
					...
					values.bindTableId = parseInt(values.bindTableId[2]);
				}
						...
						<Row gutter={24}>
							<Col span={12} >
								<FormItem label="资源分类" {...formItemLayout}>
									{getFieldDecorator("catalogIdArray", {
										initialValue: config.catalogName ? config.catalogName : "",
										rules: [{ required: true, message: "资源分类不能为空！" }],
										rules: [{ validator: this.checkId.bind(this) }]
									})(
										<Cascader
											placeholder="请选择资源分类"
								...
								<FormItem label="资源代码" {...formItemLayout}>
									{getFieldDecorator("seqNum", {
										...
										rules: [
											...
											{ pattern: /^[0-9]*$/, message: "资源代码只能阿拉伯数字！" }
										]
									})(
										<Input
							...
							<Col span={12} >
								<FormItem label="资源摘要" {...formItemLayout}>
									...
									})(<TextArea placeholder="请填写资源摘要" maxLength={500} autosize />)}
								</FormItem>
							</Col>
							<Col span={12} >
							...
							<Col span={12} style={{ display: controlVisible === "other" ? "none" : "block" }} >
								{controlVisible === "info" ? (
									<FormItem label="自定义格式描述" {...formItemLayout}>
										{getFieldDecorator("formatInfoExtend", {
											initialValue: config.formatInfoExtend
												...
												: "",
											rules: [{ required: true, message: "自定义格式描述不能为空" }]
									...
									<FormItem {...formItemLayout} label="物理表名" hasFeedback>
										{getFieldDecorator("bindTableId", {
											...
											rules: [
												...
												{ validator: checkIfTable, message: "请选择对应的物理表或数据库" }
											]
										})(
											<Cascader
												placeholder="请选择物理表名"
											...
											<FormItem label="共享条件" {...formItemLayout}>
												{getFieldDecorator("shareCondition", {
													initialValue: config.shareCondition
														...
														: "",
													rules: [{ required: true, message: "共享条件不能为空！" }]
									...
									<Col span={12} >
										<FormItem {...formItemLayout} label="共享方式" hasFeedback>
											{getFieldDecorator("shareMethod", {
												initialValue: config.shareMethod ? config.shareMethod + "" : "2",
												rules: [{ required: true, message: "共享方式不能为空！" }]
											})(
												<Select placeholder="请选择共享方式">
													{
														ResourceDict.map(index => <Option value={index.code}>{index.name}</Option>)
													}
												</Select>
											)}
										</FormItem>
							...
							<Col span={12} >
								<FormItem {...formItemLayout} label="是否向社会开放">
									{getFieldDecorator("openType", {
										...
										rules: [{ required: true, message: "请选择是否向社会开放！" }]
									})(
										<RadioGroup size={"small"} onChange={this.handleOpenChange.bind(this)}>
								...
								<Col span={12} >
									<FormItem label="开放条件" {...formItemLayout}>
										{getFieldDecorator("openCondition", {
											initialValue: config.openCondition ? config.openCondition : "",
							...
							<Row style={{ margin: "10px 0", width: "100%" }}>
								<Col span={12}>
									<ButtonGroup size={"small"}>
										...
										<Button onClick={this.handleDelete.bind(this)}>删除行</Button>
									</ButtonGroup>
								</Col>
