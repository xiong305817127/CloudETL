...
import { API_BASE_SECURITY, API_BASE_METADATA, API_BASE_MONITOR } from '../constants';
...
export async function disalbeTenant(obj){
  ...
  return request(`${API_BASE_SECURITY}/renter/updateStatus.shtml?${qs.stringify(obj)}`,option);
}
...
export async function resetPassword(obj){
  ...
  return request(`${API_BASE_SECURITY}/renter/restRenterPassword.shtml?${qs.stringify(obj)}`,option);
}
...
export async function getServicesList() {
   ...
   * 由运维系统转为metacube的接口
   */
  return request(`${API_BASE_METADATA}/ambari/api/v1/service_config_versions`, option);
