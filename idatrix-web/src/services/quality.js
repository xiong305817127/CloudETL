...
const runTrans = {
 ...
 export async function Getupdate(obj) {
  ...
  export async function GetdictAllList(query,obj) {
    return request(`${API_BASE_QUALITY}/analysis/dictDataList/${query.name}.do?`+"page="+query.page,"size="+query.size)
 }
...
export async function getAnalysisReports(obj, n) {
    ...
    switch (n) {
        case "List":
            node = runTrans.getRecordList; break;
        case "Record":
            node = runTrans.getRecordInfo; break;
        case "Result":
            node = runTrans.getResultInfo; break;
        default:
            return;
    }
...
export async function getAnalysisReportsByNode(obj) {
    ...
    const urlQuery = url + runTrans.getAnalysisReportByNode + "?" + qs.stringify(obj);
...
export async function getDefaultEngineList(query) {
    const queryString = qs.stringify(query);
    return request(`${API_BASE_QUALITY}/cloud/getDefaultEngineList.do?${queryString}`)
}
...
export async function get_db_table(query) {
    ...
    return request(`${API_BASE_QUALITY}/db/getDbTables.do`, option)
}
/*获取表字段*/
export async function get_db_table_fields(query) {
    ...
    return request(`${API_BASE_QUALITY}/db/getDbTableFields.do`, option)
}
/*查询权限*/
...
export async function get_db_schema(query) {
    ...
    return request(`${API_BASE_QUALITY}/db/getDbSchema.do`, option)
}
/*服务器列表*/
