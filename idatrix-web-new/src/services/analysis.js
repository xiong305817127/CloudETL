...
const RequestUrl={
  ...
  //8.存储列表查询：数据查询db  最新接口  2019-3-19
  DataResourceApi:'/db/resource',
  //9.语句执行：     原接口
...
export async function getDataNewList() {
  return request(RequestUrl.CommonsUrlApi + RequestUrl.DataResourceApi);
};
/* 2.2语句执行:*/
...
export async function getSchemaInfo(query) {
  ...
  return request(`${API_BASE_ANALYSIS}/db/tablesAndFields?${querystring}`);  //原来的 schemaInfo
};
