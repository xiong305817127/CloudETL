...
export async function getDepartments(query) {
  ...
  return request(`${API_BASE_METADATATOSWAP}/findDeptByUserId?${querystring}`);
}
...
export async function getHdfsTree(formData) {
  ...
  return request(`${API_BASE_METADATATOSWAP}/search`, option);
}
...
export async function getPermitsList(query) {
  const queryString = qs.stringify(query);
  return request(
    `${API_BASE_RESOURCE}/authority/getAllAuthority?${queryString}`
  );
}
