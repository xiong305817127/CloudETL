...
const index = ({ dispatch, form, biDatamodel }) => {
  ...
  const handleSubmit = e => {
    ...
    form.validateFields((err, values) => {
      ...
      if (!err) {
        if (foldId) {
          let id = values.dataSource[0];
          let dsId = values.dataSource[1];
          let databaseName = "";
          ...
          options.forEach(index => {
            if (index.value === id) {
              index.children.forEach(item => {
                if (item.value === dsId) {
                  databaseName = item.label.split("(")[0];
                }
              });
            }
          });
          ...
          dispatch(
            routerRedux.push(`/analysis/DataModel/Config?${querystring}`)
          );
        } else {
  ...
  return (
    <Modal
      ...
      footer={[
        <Button key="new" onClick={handeNewFolder} style={{ float: "left" }}>
          新建文件夹
        </Button>,
        <Button type="primary" key="sure" onClick={handleSubmit}>
          确定
        </Button>,
...
export default connect(({ biDatamodel }) => ({ biDatamodel }))(
  Form.create()(index)
);
