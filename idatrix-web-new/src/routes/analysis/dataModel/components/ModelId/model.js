...
export default {
  ...
  effects: {
    ...
    *getTableList({ payload, init }, { put, call }) {
      const { data } = yield call(getTableList, payload.dsId);
      const { code } = data;
    ...
    *addMainTable({ payload }, { put, call }) {
      const { data } = yield call(getFieldsList, payload.id);
      const { code } = data;
      if (code === "200") {
        const fields = data.data.map(index => {
          let obj = {};
          obj.name = index.columnName;
          obj.visible = true;
          obj.column = index.columnName;
          obj.id = index.id;
    ...
    *submitData({}, { put, select, call }) {
      ...
      if (
        measure.length === 0 ||
        dimension.some(index => index.Level.length === 0)
      ) {
      ...
      if (
        !measure.some(index => index.visible) ||
        !dimension.some(index => index.visible)
      ) {
  ...
  reducers: {
    ...
    deleteItem(state, action) {
      ...
      return {
        ...
        lines: lines.filter(
          index => index.targetId !== id && index.sourceId !== id
        ),
        dimension: newDimension,
        measure: newMeasure,
        mainItem: newMainItem,
        mainTableId: newMainTableId,
        mainDataSource: newMainDataSource,
        shouldUpdate: true
      };
    },
    //添加主键
