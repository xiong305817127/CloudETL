...
import { getNew_trans, getDelete_trans } from "../../../../../services/gather";
import * as FuncsList from "../../../../../services/gather";
import { CurryFunc } from "../../../../../utils/utils";

export default {
  namespace: "transheader",
  state: {
    model: "welcome",
    activeArgs: new Map(),
    activeKey: "",
  ...
  effects: {
    *deleteTrans({ payload }, { select, call, put }) {
      ...
      const { data } = yield getDelete_trans({
        name: payload.removeKey,
        owner
      });
      const { code } = data;
      if (code === "200") {
    ...
    *saveNewTrans({ payload }, { select, call, put }) {
      ...
      const { data } = yield getNew_trans({ ...payload, group: "default" });
      const { code } = data;
      console.log(payload);
