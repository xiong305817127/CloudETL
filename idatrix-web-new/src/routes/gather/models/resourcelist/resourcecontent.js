...
export default {
  ...
  effects: {
    ...
    *queryDefaultEngine({ payload }, { select, call, put }) {
      const { username } = yield select(state => state.account);
      const { data } = yield getDefaultEngineList({
        ...payload,
        owner: username
      });
      const { code } = data;
    ...
    *querySparkList({ payload }, { select, call, put }) {
    ...
    //获取schema列表
    *getSchemaList({ payload }, { select, call, put }) {
      const { username } = yield select(state => state.account);
      const { schemaList } = yield select(state => state.resourcecontent);
      if (schemaList[`${payload.name}_${payload.id}`]) return;
      const { data } = yield call(get_db_schema, {
        ...payload,
        owner: username
      });
      const { code } = data;
      if (code === "200" && data.data instanceof Array) {
        yield put({
          ...
          payload: {
            schemaList: {
              ...schemaList,
              [`${payload.id}`]: data.data.map(index => {
                index.status = "init";
                return index;
              })
            }
    ...
    *testAllDb({ payload }, { select, call, put }) {
      const { schemaList } = yield select(state => state.resourcecontent);
      yield put({
        type: "save",
        payload: {
					...schemaList,
          schemaList: {
            [payload.id]: schemaList[payload.id].map(index => {
      ...
      for (let index of schemaList[payload.id]) {
        const { data } = yield call(test_DbConnection, {
          owner: payload.owner,
          id: index.schemaId
        });
        const { code } = data;
        if (code === "200" && data.data) {
          allData.push({
            ...data.data,
            owner: payload.owner,
            status: data.data.status === 0 ? "success" : "failed",
    ...
    *testSingleDb({ payload }, { call,select,put }) {
			const { schemaList } = yield select(state => state.resourcecontent);
      yield put({
        type: "save",
        payload: {
          schemaList: {
						...schemaList,
            [payload.databaseId]: schemaList[payload.databaseId].map(index => {
							if(index.schemaId === payload.id){
								index.status = "loading";
							}
              return index;
            })
				...
				yield put({
					...
					payload: {
						schemaList: {
							...
							[payload.databaseId]: schemaList[payload.databaseId].map(index => {
								if(index.schemaId === payload.id){
									index.status = data.data.status === 0 ? "success" : "failed";
								}
								return index;
							})
						}
