...
export default {
  namespace: "controltransplatform",
  state: {
    visible: false,
    /*打开的转换名*/
    transName: "",
  ...
  effects: {
    *openTrans({ payload }, { select, call, put }) {
      const { data } = yield getOpen_trans({
    ...
    *stopTrans({ payload }, { select, call, put }) {
      const data1 = yield getTrans_exec_id(payload.actionName);
      if (data1.data.code === "200") {
        const data2 = yield get_exec_stop(data1.data.data.executionId);
