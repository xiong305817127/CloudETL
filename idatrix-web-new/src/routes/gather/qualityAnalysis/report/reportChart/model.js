 ...
 */
import { get_TransRecords,getAnalysisReports , getAnalysisReportsByNode,getRedundanceDetail  } from 'services/quality';
          ...
          yield put({
            ...
            option: {
              execId: option.execId,
							...
							name:result[0]["analysisName"],
							nodType:result[0]["nodType"],
              nodeList: result
            }
          })
        }
    ...
    *getNodeDetails({option},{call, put, select}){
      const data = yield call(
        getAnalysisReportsByNode,
        {
          ...
          nodId: option.nodId,
          isList: false, // 此处默认false，采用对象方式遍历,
      ...
      if(data && typeof data.data.data !== "undefined"){
        ...
        const nodeFullData = yield select(state=>state.analysisReports.get("nodeFullData"))
				...
				if(option.nodType === "REDUNDANCE"){
					 const redundanceData = yield call(getRedundanceDetail,{ execId:option.execId,nodId:option.nodId});
					 if(redundanceData && redundanceData.data.data){
							data.data.data[Object.keys(data.data.data)[0]].dataredundanceData = redundanceData.data.data;
					 }
      ...
      if(newSelected.length < selectedNodes.size){
        ...
        for(let nodId of newSelected){
          if(!nodeFullData.has(nodId)){
						...
						const nodeList = yield select((state)=>state.analysisReports.get("nodeList"));
						const execId = yield select(state=>state.analysisReports.get("execId"));
						const nodType = nodeList.filter(index=>index.get("nodId") === nodId).first().get("nodType");
            ifNoData = 1;
            yield put({
              type: "getNodeDetails", 
              option:{
