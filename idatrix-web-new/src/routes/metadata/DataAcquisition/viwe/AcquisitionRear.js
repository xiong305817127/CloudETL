...
class AcquisitionRear extends React.Component{
  state = {
      current: 0,
      visible: false,
      info:{},
  ...
  handleGetName = (rule,value, callback) => {
   ...
   handleRearChonel(record){
        ...
        obj.dsId = acquisition.datalist[0].dsId;
        obj.sourceId = 3;
	                  ...
	                  type:"acquisition/setMetaIdUpdata",
	                    payload:{
                           ...
                           selectedRows:acquisition.selectedRows,
                            ...
                            data1:data1,
                            dsIdData:acquisition.datalist[0].dsId,
	                    }
	                });
  ...
  /* componentDidUpdate(){
   	 Response.Write("<script language=javascript>window.location.href='/DataAcquisition'</script>")
   }
*/
 next = (e) => {
   const { dispatch }=this.props;
      ...
      case 0:
        renderShow = (
          <div className={Style.center1}>
              ...
              <TableList scroll={{y:450}} pagination={false} rowSelection={rowSelection2} onRowClick={(record)=>{console.log(record)}}  className={Style.TabelList1} dataSource={data2} columns={this.columns2()} />
          </div>
        )
        break;
         case 1:
           ...
           <div className={Style.stepsAction}>
          {
            ...
            &&
               <div>
                   ...
                   <Button type="primary" style={{ marginLeft: 8 }}> <a href={"#/DataAcquisition"}>取消</a></Button>
               </div>
          }
          {
            this.state.current === steps.length - 1
...
const AcquisitionRearForm = Form.create()(AcquisitionRear);
