...
import {
  deleteTerminalManageRecordById,
  getTerminalManageRecordsByCondition,
  saveOrUpdateTerminalManage,
  getTerminalManageRecordById,
  getDeptServer,
  getFSDatabase,
  getFSSftp,
  isExistedTerminalManageRecord
    ...
    <div>
      ...
      <Col span={8} style={{ display: "block" }}>
        <FormItem label={"数据库"} {...formItemLayout1}>
          {getFieldDecorator("dbName", {})(<Input />)}
        </FormItem>
      </Col>
    </div>
  );
...
class frontlist extends React.Component {
  ...
  require() {
    ...
    getTerminalManageRecordsByCondition(arr, {
      ...
      if (code === "200") {
        //返回结果是否成功为200则是成功
        if (data === null) {
          //data如果等于null前端会报错，所以需要再做个判断，如果为null就不返回
          dispatch({
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        if (model === "newtype") {
          ...
          isExistedTerminalManageRecord(exid)
            .then(res => {
              const { code, data, message } = res.data;
              if (code === "200") {
                let dsids = "";
                ...
                arr.sftpUsername =
                  Object.prototype.toString.call(values.sftpUsername) ===
                  "[object Array]"
                    ? ""
                    : values.sftpUsername;
  ...
  handleChangeBase = (value, label) => {
    console.log(value, label, "labelcinxi");
    const { dispatch } = this.props;
    const { database, Sid } = this.props.frontModel;
    const target = database.find(v => v.dsId == value.split(",")[1]);
    let dbPort = target.dbPort;
    let dsType = target.dsType;
          ...
          <div style={{ padding: "0 10px 0 10px" }}>
            <Row gutter={20} className="btn_std_group">
              ...
              <Col span={24} className="search_btn">
                <Button type="primary" onClick={this.require.bind(this)}>
                  {" "}
                  查询
                </Button>
                <Button
                  style={{ marginLeft: 10 }}
                  type="primary"
                  onClick={this.handletrue.bind(this)}
                >
                  {" "}
                  新增
                </Button>
              ...
              {getFieldDecorator("tmDBName", {
                ...
                >
                  {database.map(index => (
                    <Option
                      key={index.dsId}
                      value={index.dbDatabasename + "," + index.dsId}
                    >
                      {index.dbDatabasename}
                    </Option>
                  ))}
                </Select>
              )}
         ...
         */}
              ...
              <FormItem label="用户名" {...formItemLayout}>
                {getFieldDecorator("sftpUsername", {
                  ...
                  >
                    {sFTPbase.map(index => (
                      <Select.Option key={index.id} value={index.ftpUser + ""}>
                        {index.ftpUser}
                      </Select.Option>
                    ))}
                  </Select>
