  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields({ force: true }, (err, values) => {
      if (!err) {
        ...
        obj.pswd = strEnc(values.pswd,values.cardId,values.phone,values.email);   	//MD5(`#${values.pswd}`); //需要加密后传输时使用
    ...
    this.props.form.validateFields({ force: true }, (err, values) => {
      if (!err) {
        ...
        obj.realName = values.realName;
				...
				if(values.pswd === tempPassword){
					obj.pswd = strEnc(values.pswd,values.cardId,values.phone,values.email);
				}
        obj.sex = values.sex === "男" ? 1 : values.sex === "女" ? 2 : 3;
        obj.age = values.age;
        obj.email = values.email;
