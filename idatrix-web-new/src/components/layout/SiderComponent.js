...
class SiderComponent extends React.Component {
  ...
  updateStateByProps(props) {
    ...
    console.log(permitsList, "steven");
  ...
  render() {
    const { menu, system } = this.props;
    const { selectedKey, openKeys } = this.state;
    const permitsList =
      (system && system.permits && system.permits[system.currentSystemId]) ||
      [];
    console.log(menu, "菜单");

    const newMenuList = permitsList
      .filter(val => val.type === "中转站")
      .map(val => ({
        name: val.name,
        icon: "taskcenter",
