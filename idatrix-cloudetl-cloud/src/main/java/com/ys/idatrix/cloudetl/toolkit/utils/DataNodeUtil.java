...
import com.ys.idatrix.cloudetl.toolkit.analyzer.script.SQLScriptParser;
...
import com.ys.idatrix.cloudetl.toolkit.domain.property.DataItemProperty;
import com.ys.idatrix.cloudetl.toolkit.domain.property.DataSetProperty;
import com.ys.idatrix.cloudetl.toolkit.domain.property.DatabaseProperty;
import com.ys.idatrix.cloudetl.toolkit.domain.property.DummyProperty;
import com.ys.idatrix.cloudetl.toolkit.domain.property.FieldProperty;
import com.ys.idatrix.cloudetl.toolkit.domain.property.FileProperty;
import com.ys.idatrix.cloudetl.toolkit.domain.property.FileSystemProperty;
import com.ys.idatrix.cloudetl.toolkit.domain.property.InterfaceProperty;
			...
			if (!Utils.isEmpty(tableName)) {
				...
				for (int i = 0; i < rowMeta.size(); i++) {
					...
					if( Utils.isEmpty(table) ) {
						//table为空,从sql中解析获取
						if( stmt == null ) {
							stmt = SQLScriptParser.parserSelectSql(dbMeta, sql);
						}
						//sql解析字段对应的表名称 , 多表全量查询(使用 *)[select * from A,B ]样式的sql 无法解析对应表名
						//无法确认对应的表是  table还是view .
						table = SQLScriptParser.findTableByField(field.getName(), stmt);
						if (Utils.isEmpty(table)) {
							// TODO sql语义分析失败 , 打日志,忽略
							CloudLogger.getInstance().error("DataNodeUtil.dbFieldNodeParse", "sql语义分析未找到表名,域["+field.getName()+"],sql["+sql+"]");
							continue;
						}
					}
