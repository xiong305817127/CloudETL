...
import com.ys.idatrix.db.parser.SqlParserService;
...
public class SqlExecServiceImpl implements SqlExecService {
    ...
    public RespResult<SqlQueryRespDto> executeQuery(String username, SqlExecReqDto sqlExecReqDto) {
        try {
            ...
            List<ParseResultDto> parseResults = sqlParserService.parseSQL(sqlExecReqDto.getCommand(), sqlExecReqDto.getSchemaDetails().getType());
    ...
    public RespResult<SqlExecRespDto> asyncExecute(String username, SqlExecReqDto sqlExecReqDto) {
        try {
            ...
            String sqlExecType;
            if (DatabaseTypeEnum.HIVE.getName().equalsIgnoreCase(sqlExecReqDto.getSchemaDetails().getType())) {
                sqlExecType = HiveOperator.valueOf(parseResults.get(0).getMainOperator()).getRunnerType();
            } else if (DatabaseTypeEnum.HBASE.getName().equalsIgnoreCase(sqlExecReqDto.getSchemaDetails().getType())) {
            ...
            log.info("数据源类型:{}, 执行SQL:{},操作类型:{} ", sqlExecReqDto.getSchemaDetails().getType(), sqlExecReqDto.getCommand(), sqlExecType);
    ...
    private SqlExecRespDto asyncExecute(String username, RdbLinkDto linkDto,
        ...
        dbSqlResult.setDbSource(sqlExecReqDto.getSchemaDetails().getSchemaName());
    ...
    private void wrapSqlExecuteDto(SqlExecReqDto executeDto) {
        if (executeDto.getSchemaModeEnum() != null) {

            if (executeDto.getSchemaModeEnum() == SchemaModeEnum.id) {

                if (null == executeDto.getSchemaId()) {
                    throw new DbProxyException("模式ID为空");
                }
                ...
                if (dbRet.isSuccess()) {
                    ...
                    SchemaDetailsDto schemaDetailsDto = new SchemaDetailsDto();
                    schemaDetailsDto.setSchemaName(dbDTO.getDbName());
                    schemaDetailsDto.setIp(dbDTO.getIp());
                    schemaDetailsDto.setPort(dbDTO.getPort());
                    schemaDetailsDto.setUsername(dbDTO.getUsername());
                    schemaDetailsDto.setPassword(dbDTO.getPassword());
                    schemaDetailsDto.setType(dbDTO.getType());
                    executeDto.setSchemaDetails(schemaDetailsDto);
                } else {
            ...
            throw new DbProxyException("数据库链接方式参数为空");
        }
    }
