...
public class SqlParserService {
    ...
    private void getActualSqlOperator() {
        log.info("初始化SQL执行可执行操作：{}" , dbSqlOperator);
        if (StringUtils.isBlank(this.dbSqlOperator)) {
            actualSqlOperators = null;
        } else {
            actualSqlOperators = Maps.newHashMap();
            //db类型
    ...
    public void validateAndRebuildParseResult(List<ParseResultDto> results, SqlExecReqDto executeDto, String username, String specialOperator) throws Exception {
        if (CollectionUtils.isNotEmpty(results)) {
            for (ParseResultDto result : results) {
                ...
                //校验操作
                validateOperator(mainOperator, executeDto.getSchemaDetails().getType());
        ...
        if (MapUtils.isNotEmpty(tabOperators)) {
            for (String tableName : tabOperators.keySet()) {
                ...
                for (String operator : tbOperators) {
                    ...
                    switch (DatabaseTypeEnum.valueOf(executeDto.getSchemaDetails().getType().toUpperCase())) {
                        case MYSQL:
                        case ORACLE:
                        case DM:
                        case POSTGRESQL:
    ...
    public void rebuildSql(SqlExecReqDto executeDto, Map<String, String> tabOperators) {
        if (MapUtils.isNotEmpty(tabOperators)) {
            ...
            log.info("{} sql rebuild before:{}", executeDto.getSchemaDetails().getType(), trimSql);
            switch (DatabaseTypeEnum.valueOf(executeDto.getSchemaDetails().getType().toUpperCase())) {
                case DM:
                case POSTGRESQL:
                    for (String tableName : tabOperators.keySet()) {
                        String regex = "(\\b\\s+" + tableName + "\\b)";
                        trimSql = replaceAllIgnoreCase(trimSql, regex, KEYWORD_SPACE + executeDto.getSchemaDetails().getSchemaName() + "." + tableName);
                    }
                    break;
            ...
            log.info("{} sql rebuild after:{}", executeDto.getSchemaDetails().getType(), trimSql);
            ...
            if (actualRowLimit == -1) {
                switch (DatabaseTypeEnum.valueOf(executeDto.getSchemaDetails().getType().toUpperCase())) {
                    ...
                    case POSTGRESQL:
                        ...
                        break;
                    default:
                        log.warn(WARN_NO_LIMIT_LOG, executeDto.getSchemaDetails().getType(), KEYWORD_LIMIT);
                        break;
                }
            }
            ...
            if (actualRowLimit > rowLimit) {
                switch (DatabaseTypeEnum.valueOf(executeDto.getSchemaDetails().getType().toUpperCase())) {
                    ...
                    /*case DB2:
                        ...
                        break;*/
                    default:
                        log.warn(WARN_NO_LIMIT_LOG, executeDto.getSchemaDetails().getType().toUpperCase(), KEYWORD_LIMIT);
                        break;
                }
            }
            if (null != rebuildSql) {
