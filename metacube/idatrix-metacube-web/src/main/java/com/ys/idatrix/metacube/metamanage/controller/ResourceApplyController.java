...
public class ResourceApplyController {
    ...
    @RequestMapping("/userIfAuthority/{metadataId}")
    public ResultBean<Boolean> userIfAuthority(@NotNull(message = "元数据ID不能为空") Long metadataId,
                                               @NotNull(message = "资源数据库类型不能为空") @RequestParam(value = "resourceType") Integer resourceType) {
    ...
    @RequestMapping("/authority/list/{metadataId}")
