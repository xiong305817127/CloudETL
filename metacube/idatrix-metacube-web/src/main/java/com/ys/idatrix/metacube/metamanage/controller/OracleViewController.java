...
/**
 * @ClassName OracleViewController
 * @Description oracle视图操作相关API
 * @Author ouyang
 * @Date
 */
@Slf4j
@Validated
@RestController
@RequestMapping("/oracle/view")
@Api(value = "/OracleViewController", tags = "元数据管理-元数据定义-ORACLE视图处理接口")
public class OracleViewController {
    ...
    @ApiOperation(value = "元数据定义-ORACLE视图搜索", notes="传递参数schemaId,status", httpMethod = "GET")
    @ApiImplicitParams({
    ...
    @ApiOperation(value = "元数据定义-ORACLE根据ID查询视图", notes="", httpMethod = "GET")
    @GetMapping("/search/{viewId}")
    public ResultBean<ViewVO> searchById(@NotNull(message = "视图ID不能为空") @PathVariable Long viewId) {
        ViewVO result = viewService.searchById(viewId);
        return ResultBean.ok(result);
    }

    @ApiOperation(value = "元数据定义-ORACLE视图保存并生效", notes = "新增数据时，尝试生效数据库中", httpMethod = "POST")
    @ApiImplicitParam(name = "view", value = "oracle view 实体类", dataType = "DBViewVO", paramType = "body")
    @PostMapping("/add")
    public ResultBean add(@Validated(Save.class) @RequestBody DBViewVO view, BindingResult bindingResult) {
        ...
        log.info("新增 oracle 视图数据：{}", view);
        viewService.add(view);
        return ResultBean.ok();
    }

    @ApiOperation(value = "元数据定义-ORACLE视图修改并生效", notes = "修改数据时，尝试生效到数据库中", httpMethod = "PUT")
    @ApiImplicitParam(name = "view", value = "oracle view 实体类", dataType = "DBViewVO", paramType = "body")
    @PutMapping("/update")
    public ResultBean update(@Validated(Update.class) @RequestBody DBViewVO view, BindingResult bindingResult) {
        ...
        log.info("修改 oracle 视图数据：{}", view);
        viewService.update(view);
        return ResultBean.ok();
    }

    @ApiOperation(value = "元数据定义-ORACLE视图存为草稿", notes = "存为草稿，并不去生效到数据库中", httpMethod = "POST")
    @ApiImplicitParam(name = "view", value = "oracle view 实体类", dataType = "DBViewVO", paramType = "body")
    ...
    public ResultBean addDraft(@Validated(Save.class) @RequestBody DBViewVO view, BindingResult bindingResult) {
        ...
        log.info("保存 oracle 视图草稿：{}", view);
        viewService.addDraft(view);
        return ResultBean.ok();
    }

    @ApiOperation(value = "元数据定义-ORACLE视图草稿修改", notes = "草稿修改并不生效，生效表存为草稿", httpMethod = "PUT")
