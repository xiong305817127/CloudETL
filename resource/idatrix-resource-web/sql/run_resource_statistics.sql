-- 更新资源统计数据到表格 rc_resource_statistics
-- 请在以下情况下运行：
-- 1. 统计数据错误，需要强制更新时候，运行此脚本
-- 2. 程序已经运行一定时间，已经产生订阅、交换数据但未同步到统计表中。
-- 统计方式： 
...
UPDATE rc_resource_statistics rrs,
(
SELECT
	orgg_id.id AS id,
...
GROUP BY
	...
	SET rrs.sub_count = val_tab.sub_count,
	rrs.data_count = val_tab.data_count,
	rrs.share_data_count = val_tab.share_data_count,
	rrs.data_update_time = val_tab.data_update_time 
WHERE
	rrs.id = val_tab.id;