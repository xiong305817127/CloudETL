...
import com.idatrix.resource.datareport.vo.DataUploadDetailVO;
...
import com.ys.idatrix.db.agent.api.hdfs.HdfsExecuteResult;
import com.ys.idatrix.db.proxy.api.hdfs.HdfsUnrestrictedDao;
import org.apache.commons.lang3.StringUtils;
import org.poi.ExcelReaderUtil;
...
public class DataUploadServiceImpl implements IDataUploadService {
	...
	public DataUploadServiceImpl(DataUploadDAO dataUploadDAO,
								 ...
								 HdfsUnrestrictedDao hdfsUnrestrictedDao,
	...
	private HdfsUnrestrictedDao hdfsUnrestrictedDao;
	...
	public Long saveOrUpdateUploadDataForDB(Long resourceId, String dataBatch, Integer formatType,
...
//		SystemConfigPO systemConfigPO = systemConfigDAO.getLastestSysConfig();
		...
		if (systemConfigPO == null)
			throw new CommonServiceException(CommonConstants.EC_NULL_VALUE, "系统配置参数不存在");
		...
		Long parentId = saveDataUploadRecord(resourceId, dataBatch, CommonConstants.DATA_TYPE_DB, userName);
...
//			SystemConfigPO systemConfigPO = systemConfigDAO.getLastestSysConfig();
	...
	private Long uploadFileToHDFS(CommonsMultipartFile file, String dataType, String hdfsPath, String uniqueFileName)
		...
		try {
			...
			HdfsExecuteResult hdfsExecuteResult =
			...
			if (!hdfsExecuteResult.isSuccess()) {
				...
				LOG.error(errMsg + " " + hdfsExecuteResult.getMessage());
	...
	private Long saveDataUploadRecord(Long resourceId, String dataBatch, String dataType, String userName)
			throws RuntimeException, CommonServiceException {
		Long taskSeq;
		try {
	...
	public void downLoadExcelTemplate(Long resourceId, HttpServletResponse response)
		...
		try {
			...
			if (columnPOList == null || columnPOList.isEmpty()) {
	...
	public ResultPager<SearchDataUploadVO> getDataUploadRecordByCondition(Map<String, String> conditionMap,
		...
		if (poList != null && !poList.isEmpty()) {
			...
			return new ResultPager<SearchDataUploadVO>(pi.getPageNum(),
					totalNum, voList);
		}
	...
	@Override
	public void updateUploadDataForFILE(DataUploadTotalVO dataUploadTotalVO, String userName)
			throws CommonServiceException {
		//如果上传成功, 生成数据上报rc_data_upload记录
		Long parentId = saveDataUploadRecord(dataUploadTotalVO.getResourceId(), dataUploadTotalVO.getDataBatch(),
				dataUploadTotalVO.getDataType(), userName);

		if (dataUploadTotalVO.getDataUploadDetailVOList() != null
				&& !dataUploadTotalVO.getDataUploadDetailVOList().isEmpty()) {
...
//		try {
//			InputStream inputStream = new FileInputStream(localTmpPath);
//
//			if (isExcel2003(localTmpPath)) {// 当excel是2003时,创建excel2003
//				HSSFWorkbook wb = new HSSFWorkbook(inputStream);
//				HSSFSheet sheet = wb.getSheetAt(0);
//				return (long)sheet.getLastRowNum();//获得总行数
...
//			LOG.error(new Date() + "未发现保存到本地临时目录的目标文件" + localTmpPath);
//			throw new CommonServiceException(CommonConstants.EC_UNEXPECTED, "未发现保存到本地临时目录的目标文件" + localTmpPath);
//		} catch (IOException e) {
//			LOG.error(new Date() + "读取本地临时文件时出现IO异常" + localTmpPath);
//			throw new CommonServiceException(CommonConstants.EC_UNEXPECTED, "读取本地临时文件时出现IO异常" + localTmpPath);
//		}

        //原有Excel poi 在数据量有5万行以上时，操作会导致内存溢出 robin 2018/08/24
        int rowNums = 0;
        try{
            rowNums = ExcelReaderUtil.readExcel(localTmpPath);
        }catch (Exception e){
            e.printStackTrace();
