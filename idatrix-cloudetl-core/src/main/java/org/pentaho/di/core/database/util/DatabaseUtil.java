...
public class DatabaseUtil implements DataSourceProviderInterface {
  ...
  public DataSource getNamedDataSource( String datasourceName ) throws DataSourceNamingException {
    ...
    try {
      ...
      String root = (String) OsgiBundleUtils.invokeOsgiMethod(Utils.getCloudApp(), "getRepositoryRootFolder"); 
      root = Utils.isEmpty(root) ? (IdatrixPropertyUtil.getProperty("idatrix.metadata.reposity.root","/data/ETL/reposity/")+"Jndi/"):root ;
      
      String path = (String) OsgiBundleUtils.invokeOsgiMethod(Utils.getCloudSession(), "getUserJndiRepositoryPath");
      path= Utils.isEmpty(path)?"jndi/":path;
      
      Hashtable<String, Object> env = new  Hashtable<String,Object>();
      env.put("org.osjava.sj.root", root+path);
     
      return DatabaseUtil.getDataSourceFromJndi( datasourceName, new InitialContext(env) );
