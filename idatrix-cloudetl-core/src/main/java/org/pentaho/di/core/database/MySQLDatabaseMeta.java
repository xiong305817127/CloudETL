...
//        }
      ...
      case ValueMetaInterface.TYPE_STRING:
        if ( length > 0 ) {
          if ( length == 1 ) {
            retval += "CHAR(1)";
          } else if ( length < 2048 ) {  //xionghan 原生 小于256 ,太小
    ...
    if ( fieldname.equalsIgnoreCase( tk ) || // Technical key
            ...
            if ( use_autoinc ) {
              retval += " AUTO_INCREMENT NOT NULL PRIMARY KEY";
            } else {
  ...
  public void addDefaultOptions() {
	...
	//xionghan jdbc:mysql 方式的url排除使用mariadb数据库
    addExtraOption( getPluginId(), "disableMariaDbDriver", "true" );
  }
}
