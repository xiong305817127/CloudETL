...
public class Database implements VariableSpace, LoggingObjectInterface {
  ...
  public void normalConnect( String partitionId ) throws KettleDatabaseException {
    ...
    try {
      ...
      if ( databaseMeta.getAccessType() == DatabaseMeta.TYPE_ACCESS_JNDI ) {
        ...
        try {
        	//xionghan 增加 databaseMeta.getPluginId()+"/jdbc/" 的数据源前缀
          this.connection = dsp.getNamedDataSource( databaseMeta.getPluginId()+"/jdbc/"+jndiName, DatasourceType.JNDI ).getConnection();
    ...
    PluginInterface plugin =
      PluginRegistry.getInstance().getPlugin( DatabasePluginType.class, databaseMeta.getDatabaseInterface() );
		...
		if ( databaseMeta.isPartitioned() && !Utils.isEmpty( partitionId ) ) {
			databaseName = databaseMeta.getPartitionMeta( partitionId ).getDatabaseName();
		} else {
			databaseName = environmentSubstitute( databaseMeta.getDatabaseName() );
		}
		
		// [Changed for iDatrix] 查找DatabaseInterface是否有getConnection(String database)方法, 有就调用!
		DatabaseInterface dataInterface = databaseMeta.getDatabaseInterface();
		Object conn = OsgiBundleUtils.invokeOsgiMethod(dataInterface, "getConnection", databaseName);
		if(conn != null){
			log.logBasic("Got the connection by calling getConnection() from DatabaseInterface: " + dataInterface.getName());
  ...
  public String getInsertStatement( String schemaName, String tableName, RowMetaInterface fields ) {
    ...
    String schemaTable = databaseMeta.getQuotedSchemaTableCombination( schemaName, tableName );
    	// [Changed for iDatrix] 查找DatabaseInterface是否有getInsertSql(String schemaTable, RowMetaInterface fields, Object[] values )方法, 有就调用!
		DatabaseInterface dataInterface = databaseMeta.getDatabaseInterface();
		Object sql = OsgiBundleUtils.invokeOsgiMethod(dataInterface, "getInsertSql", schemaTable,fields,new Object[0]);
		if(!Utils.isEmpty((String)sql)){
			log.logBasic("Got the connection by calling getInsertSql() from DatabaseInterface: " + dataInterface.getName());
			return sql.toString();
		}
      ...
      if ( params != null ) {
        ...
        pstmt =
          connection.prepareStatement(
            databaseMeta.stripCR( sql ), Integer.valueOf(getVariable("idatrix.database.query.resultSetType", ""+ResultSet.TYPE_FORWARD_ONLY)), ResultSet.CONCUR_READ_ONLY );
        ...
        sel_stmt = connection.createStatement(Integer.valueOf(getVariable("idatrix.database.query.resultSetType", ""+ResultSet.TYPE_FORWARD_ONLY)), ResultSet.CONCUR_READ_ONLY);
  ...
  public RowMetaInterface getQueryFieldsFromDatabaseMetaData() throws Exception {
    ...
    while ( columns.next() ) {
      ...
      if ( valueMeta != null ) {
        ...
        //是否可以为空,是否自增
        valueMeta.setOriginalNullable(columns.getInt("NULLABLE" ));
        valueMeta.setOriginalAutoIncrement(Boolean.valueOf(columns.getString("IS_AUTOINCREMENT")));
  ...
  public boolean prepareUpdate( String schemaName, String tableName, String[] codes, String[] condition,
    ...
    try {
      ...
      String schemaTable = databaseMeta.getQuotedSchemaTableCombination( schemaName, tableName );
      		// [Changed for iDatrix] 查找DatabaseInterface是否有getUpdateSql(String schemaTable, String[] codes, String[] condition, String[] sets )方法, 有就调用!
			DatabaseInterface dataInterface = databaseMeta.getDatabaseInterface();
			Object sqlObj = OsgiBundleUtils.invokeOsgiMethod(dataInterface, "getUpdateSql", schemaTable, codes,condition,sets);
			if(!Utils.isEmpty((String)sqlObj)){
				log.logBasic("Got the connection by calling getUpdateSql() from DatabaseInterface: " + dataInterface.getName());
				sql.append( sqlObj.toString() );
			}else {
  ...
  public String getSQLOutput( String schemaName, String tableName, RowMetaInterface fields, Object[] r,
    ...
    try {
      String schemaTable = databaseMeta.getQuotedSchemaTableCombination( schemaName, tableName );
      		// [Changed for iDatrix] 查找DatabaseInterface是否有getInsertSql(String schemaTable, RowMetaInterface fields, Object[] values )方法, 有就调用!
			DatabaseInterface dataInterface = databaseMeta.getDatabaseInterface();
			Object sql = OsgiBundleUtils.invokeOsgiMethod(dataInterface, "getInsertSql", schemaTable,fields, r);
			if(!Utils.isEmpty((String)sql)){
				log.logBasic("Got the connection by calling getInsertSql() from DatabaseInterface: " + dataInterface.getName());
				return sql.toString();
			}
  ...
  public Object getParentObject() {
	...
	public String[] getPrimaryKeyColumnNames( String tablename ) throws KettleDatabaseException {
		return getPrimaryKeyColumnNames(null, tablename);
	}
