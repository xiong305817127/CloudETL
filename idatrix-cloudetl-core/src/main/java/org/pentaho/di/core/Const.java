...
public class Const {
   ...
   * @return number in a string.
   */
  public static String getDateStringOnly( String input ,String conversionMask ) {
	    if ( Utils.isEmpty( input ) ) {
	      return null;
	    }
	    ...
	    conversionMask = conversionMask.trim() ;
	    StringBuilder maskOnly = new StringBuilder();
	    List<Character> dateDFlag = Lists.newArrayList( 'y','d','k','H','m','s','S','D','F','w','W','h','K','Y','u','X','L'); //数字型
	    List<Character> dateWFlag = Lists.newArrayList( 'G','M','E','a','z','Z'); //字符型
	    
	    char oldChar = ' ';
	    boolean isQuotation = false;
	    maskOnly.append(".*?(");
		for ( int i = 0; i < conversionMask.length(); i++ ) {
	    	char m = conversionMask.charAt(i);
			if( i > 0 ) {
	    		oldChar =conversionMask.charAt(i-1);
	    	}
			...
			if( oldChar != '\\' && ( m == '\'' || m == '"') ) {
				...
				//引号引起了的字符串
				maskOnly.append(m);
			}else if(m == oldChar && ( dateDFlag.contains(m) || dateWFlag.contains(m)) ) {
	    		//相同的关键字符
	    		continue ;
	    	}else if( dateDFlag.contains(m) ) {
	    		//数字型 字符
	    		maskOnly.append("(\\d+)");
	    	}else if( dateWFlag.contains(m) ) {
	    		//字符型 字符
	    		maskOnly.append("(\\w+)");
	    	}else {
	    		//自定义字符
		 ...
		 Pattern pattern = Pattern.compile(maskOnly.toString());
