...
/**
 * Utilities for meta store operation on spark run configuration.
 * @author JW
 * @since 2017年6月30日
 *
 */
public class SparkEngineMetaStoreUtil extends MetaStoreUtil {
	
	public static List<SparkEngineMeta> getElements(IMetaStore metaStore) throws MetaStoreException {
		List<SparkEngineMeta> configs = new ArrayList<>();

		// If the data type doesn't exist, it's an empty list...
		//
		IMetaStoreElementType elementType = metaStore.getElementTypeByName(
				MetaStoreConst.NAMESPACE_IDATRIX, MetaStoreConst.ELEMENT_TYPE_NAME_SPARK_RUN_CONFIG);
		if (elementType == null) {
			return configs;
		}
		...
		for (IMetaStoreElement element : elements) {
			try {
				SparkEngineMeta config = loadMetaFromElement(metaStore, element);
				configs.add(config);
			} catch (Exception e) {
	...
	public static SparkEngineMeta getElement(IMetaStore metaStore, String name) throws MetaStoreException {
		// If the data type doesn't exist, it's an empty list...
		//
		IMetaStoreElementType elementType = metaStore.getElementTypeByName(
				MetaStoreConst.NAMESPACE_IDATRIX, MetaStoreConst.ELEMENT_TYPE_NAME_SPARK_RUN_CONFIG);
	...
	public static void createElement(IMetaStore metaStore, SparkEngineMeta sparkMeta) throws MetaStoreException {
	...
	public static IMetaStoreElementType populateElementType(IMetaStore metaStore) throws MetaStoreException {
		...
		IMetaStoreElementType elementType =
				metaStore.getElementTypeByName(MetaStoreConst.NAMESPACE_IDATRIX, MetaStoreConst.ELEMENT_TYPE_NAME_SPARK_RUN_CONFIG);
		if (elementType != null) {
			return elementType;
		}
		...
		//
		elementType.setName(MetaStoreConst.ELEMENT_TYPE_NAME_SPARK_RUN_CONFIG);
		elementType.setDescription(MetaStoreConst.ELEMENT_TYPE_DESCRIPTION_SPARK_RUN_CONFIG);
		metaStore.createElementType(MetaStoreConst.NAMESPACE_IDATRIX, elementType);
		return elementType;
	}

	public static IMetaStoreElement populateElement(IMetaStore metaStore, SparkEngineMeta sparkMeta) throws MetaStoreException {
		if (!metaStore.namespaceExists(MetaStoreConst.NAMESPACE_IDATRIX)) {
			throw new MetaStoreException("Namespace '" + MetaStoreConst.NAMESPACE_IDATRIX + "' doesn't exist.");
		}
		
		if (sparkMeta.getName() == null) {
			throw new MetaStoreException("Spark engine name '" + MetaStoreConst.NAMESPACE_IDATRIX + "' can't be empty.");
		}
	...
	public static SparkEngineMeta loadMetaFromElement(IMetaStore metaStore, IMetaStoreElement element) throws KettlePluginException {
		SparkEngineMeta sparkMeta = new SparkEngineMeta();
	...
	public static void updateElement(IMetaStore metaStore, SparkEngineMeta sparkMeta) throws MetaStoreException {
		// Populate element
	...
	public static void deleteElement(IMetaStore metaStore, SparkEngineMeta sparkMeta) throws MetaStoreException {
		IMetaStoreElementType elementType = metaStore.getElementTypeByName(
				MetaStoreConst.NAMESPACE_IDATRIX, MetaStoreConst.ELEMENT_TYPE_NAME_SPARK_RUN_CONFIG);
		if (elementType == null) {
			return;
