...
public class KettleDatabaseRepository extends KettleDatabaseRepositoryBase {
  ...
  public List<DatabaseMeta> getDatabases() throws KettleException {
    ...
    //xionghan 获取自定义的数据库
    try {
		list.addAll( DatabaseMetaStoreUtil.getDatabaseElements(metaStore) );
  ...
  public DatabaseMeta loadDatabaseMetaFromStepAttribute( ObjectId idStep, String code, List<DatabaseMeta> databases ) throws KettleException {
    ...
    //xionghan 从自定义的数据库里获取
    DatabaseMeta result = DatabaseMeta.findDatabase( databases, new LongObjectId( id_database ) ) ;
    if(result ==  null ) {
    	try {
			result =  DatabaseMetaStoreUtil.getDatabaseElementById(metaStore, id_database+"") ;
		} catch (MetaStoreException e) {
		}
    }
