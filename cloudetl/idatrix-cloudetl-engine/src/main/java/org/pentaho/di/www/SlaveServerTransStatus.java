...
public class SlaveServerTransStatus {
  ...
  private List<SegmentingPartInfo> partLog;
  ...
  public String getXML() throws KettleException, IOException {
    ...
    //xionghan cloud 日志
    xml.append( "  " ).append( XMLHandler.addTagValue( "cloudLog", XMLHandler.buildCDATA(  cloudLog!=null?HttpUtil.encodeBase64ZippedString(cloudLog ):"" )  ) );
    
	if( partLog != null && partLog.size() >0) {
		xml.append("  ").append(XMLHandler.openTag(SegmentingPartInfo.XML_TAGS)).append(Const.CR);
		for(SegmentingPartInfo  pl : partLog ) {
			xml.append(pl.getXML());
		}
		xml.append("  ").append(XMLHandler.closeTag(SegmentingPartInfo.XML_TAGS)).append(Const.CR);
	}
  ...
  public SlaveServerTransStatus( Node transStatusNode ) throws KettleException {
    ...
    //xionghan cloud 日志
    String cloudLog64 = XMLHandler.getTagValue( transStatusNode, "cloudLog" );
    if ( !Utils.isEmpty( cloudLog64 ) ) {
        // This is a CDATA block with a Base64 encoded GZIP compressed stream of data.
        String dataString64 =  cloudLog64.substring( "<![CDATA[".length(), cloudLog64.length() - "]]>".length() );
        try {
        	cloudLog = HttpUtil.decodeBase64ZippedString( dataString64 );
        } catch ( IOException e ) {
        	cloudLog =  "Unable to decode logging from remote server : " + e.toString() + Const.CR + Const.getStackTracker( e );
        }
    } else {
    	  cloudLog = "";
    }
    
    Node partsNode = XMLHandler.getSubNode(transStatusNode,SegmentingPartInfo.XML_TAGS);
	if(partsNode != null ) {
		partLog = new ArrayList<SegmentingPartInfo>();
		int n = XMLHandler.countNodes(partsNode, SegmentingPartInfo.XML_TAG);
		for (int i = 0; i < n; i++) {
			Node partNode = XMLHandler.getSubNodeByNr(partsNode,SegmentingPartInfo.XML_TAG, i);
			SegmentingPartInfo ehsp = new SegmentingPartInfo(partNode);
			partLog.add(ehsp);
		}
	}
   ...
   * @param  设置 cloudLog
   */
  public void setCloudLog(String cloudLog) {
   ...
   * @param  设置 partLog
   */
  public void setPartLog(List<SegmentingPartInfo> partLog) {
