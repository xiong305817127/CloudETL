...
public Result execute( Result previousResult, int nr ) throws KettleException {
    ...
    try {
      ...
      FileObject localFiles =null;
      if ( !copyprevious && !copypreviousfiles ) {
        // Get all the files in the local directory...
        myFileList = new ArrayList<FileObject>();

        localFiles = KettleVFS.getFileObject( realLocalDirectory, this );
            ...
            if ( children[i].getType().equals( FileType.FILE ) ) {
              ...
              myFileList.add( children[i] );
            	...
            	if(includeSubFolders) {
            		getChildFiles(myFileList, children[i], localFiles,sftpclient);
            	}
            }
          } // end for
        }
      ...
      while ( it.hasNext() && !parentJob.isStopped() ) {
        ...
        try {
          ...
          if(localFiles != null) {
        	  destinationFilename = localFiles.getName().getRelativeName(myFile.getName());
          }
  ...
  public void getChildFiles( ArrayList<FileObject> result , FileObject source , FileObject rootFiles , SFTPClient sftpclient) throws FileSystemException, KettleException {
	  if(source == null || source.isFile() ) {
		  return ;
	  }

	  String parentPath = rootFiles.getName().getRelativeName(source.getName());
	  boolean existfolder = sftpclient.folderExists( parentPath );
      if ( !existfolder ) {
        if ( !isCreateRemoteFolder() ) {
          throw new KettleException( BaseMessages.getString(  PKG, "JobSFTPPUT.Error.CanNotFindRemoteFolder",   parentPath ) );
        }
        if ( log.isDetailed() ) {
          logDetailed( BaseMessages .getString( PKG, "JobSFTPPUT.Error.CanNotFindRemoteFolder", parentPath ) );
        }
        // Let's create folder
        sftpclient.createFolder( parentPath );
        if ( log.isDetailed() ) {
          logDetailed( BaseMessages.getString( PKG, "JobSFTPPUT.Log.RemoteFolderCreated", parentPath ) );
        }
      }
        ...
        for ( int i = 0; i < children.length; i++ ) {
          ...
          if ( children[i].getType().equals( FileType.FILE ) ) {
            // myFileList.add(children[i].getAbsolutePath());
        	  ...
        	  getChildFiles(result, children[i],rootFiles, sftpclient);
          }
        } // end for
      }
  }
