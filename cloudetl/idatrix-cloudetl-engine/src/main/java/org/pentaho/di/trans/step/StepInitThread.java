  ...
  public void run() {
    ...
    try {
      ...
      if ( combi.step.init( combi.meta, combi.data ) ) {
    	...
    	//xionghan 当step初始化抛出了异常时,保存4W异常信息
      	if( IdatrixPropertyUtil.getBooleanProperty("idatrix.exec.save.init.exception", true) || "true".equalsIgnoreCase(combi.step.getVariable("idatrix.exec.save.init.exception"))) {
      		combi.step.getTrans().save4WException(null, null,  combi.stepname,  null ,combi.step.getLogChannel().getLogChannelId());
      	}
       ...
       //xionghan 当step初始化抛出了异常时,保存4W异常信息
    	if( IdatrixPropertyUtil.getBooleanProperty("idatrix.exec.save.init.exception", true) || "true".equalsIgnoreCase(combi.step.getVariable("idatrix.exec.save.init.exception"))) {
    		combi.step.getTrans().save4WException(null, null,  combi.stepname,  e ,null);
    	}
      //xionghan 当step初始化抛出了异常时,记录错误数
      combi.step.setErrors( 1 );
      log.logError( BaseMessages.getString( PKG, "Trans.Log.ErrorInitializingStep", combi.step.getStepname() ) );
