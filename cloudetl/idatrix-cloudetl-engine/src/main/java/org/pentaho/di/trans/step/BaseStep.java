...
public class BaseStep implements VariableSpace, StepInterface, LoggingObjectInterface, ExtensionDataInterface {
   ...
   * 数据计数监听器
   */
  private List<CounterDataListener> counterDataListeners;
  /**
   * 数据等待监听器
   */
  private WaitDataListener waitGetDataListener;
  private WaitDataListener waitPutDataListener;
  ...
  public void setErrors( long e ) {
	if(counterDataListeners != null && counterDataListeners.size()>0) {
		for( CounterDataListener dcl : counterDataListeners) {
			dcl.update("linesErrors", getStepname(), e);
		}
  	}
    errors = e;
  }
   ...
   */
  public long incrementLinesRead(RowSet rowSet) {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		if(rowSet == null && inputRowSets.size() > currentInputRowSetNr) {
    			rowSet = inputRowSets.get( currentInputRowSetNr );
    		}
    		if(waitGetDataListener == null || waitGetDataListener.isEnd(rowSet)) {
    			for( CounterDataListener dcl : counterDataListeners) {
    				dcl.update("linesRead", rowSet!=null?rowSet.getOriginStepName():getStepname(),  linesRead+1);
    			}
    		}
    	}
      return ++linesRead;
  ...
  public long incrementLinesRead() {
	  return incrementLinesRead(null);
  }
   ...
   * @return Returns the new value
   */
  public long decrementLinesRead(RowSet rowSet) {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		if(rowSet == null && inputRowSets.size() > currentInputRowSetNr) {
    			rowSet = inputRowSets.get( currentInputRowSetNr );
    		}
    		for( CounterDataListener counterDataListener : counterDataListeners) {
    			counterDataListener.update("linesRead",  rowSet!=null?rowSet.getOriginStepName():getStepname(), linesRead-1);
			}
  ...
  public long incrementLinesInput() {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		for( CounterDataListener counterDataListener : counterDataListeners) {
    			counterDataListener.update("linesInput", getStepname(), linesInput+1);
			}
    	}
      return ++linesInput;
    }
  ...
  public long incrementLinesOutput() {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		for( CounterDataListener counterDataListener : counterDataListeners) {
    			counterDataListener.update("linesOutput", getStepname(), linesOutput+1);
			}
    	}
      return ++linesOutput;
    }
   ...
   * @return Returns the new value
   */
  public long incrementLinesWritten(RowSet rowSet) {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		if(rowSet == null && outputRowSets.size() > currentOutputRowSetNr) {
    			rowSet = outputRowSets.get( currentOutputRowSetNr );
    		}
    		if(waitPutDataListener == null || waitPutDataListener.isEnd(rowSet)) {
    			String name = rowSet!=null?rowSet.getDestinationStepName():getStepname();
    			for( CounterDataListener counterDataListener : counterDataListeners) {
    				counterDataListener.update("linesWritten", name, linesWritten+1);
    			}
  ...
  public long incrementLinesWritten() {
	  return incrementLinesWritten(null);
  }
   ...
   * @return Returns the new value
   */
  public long decrementLinesWritten(RowSet rowSet) {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		if(rowSet == null && outputRowSets.size() > currentOutputRowSetNr) {
    			rowSet = outputRowSets.get( currentOutputRowSetNr );
    		...
    		for( CounterDataListener counterDataListener : counterDataListeners) {
    			counterDataListener.update("linesWritten", rowSet!=null?rowSet.getDestinationStepName():getStepname(), linesWritten-1);
			}
  ...
  public long decrementLinesWritten() {
	  return decrementLinesWritten(null);
  }
  ...
  public long incrementLinesUpdated() {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		for( CounterDataListener counterDataListener : counterDataListeners) {
    			counterDataListener.update("linesUpdated", getStepname(), linesUpdated+1);
			}
    	}
  ...
  public long incrementLinesRejected() {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		for( CounterDataListener counterDataListener : counterDataListeners) {
    			counterDataListener.update("linesRejected",errorRowSet!=null?errorRowSet.getDestinationStepName():getStepname(), linesRejected+1);
			}
    	}
      return ++linesRejected;
    }
  ...
  public long incrementLinesSkipped() {
    synchronized ( statusCountersLock ) {
    	if(counterDataListeners != null && counterDataListeners.size()>0) { 
    		for( CounterDataListener counterDataListener : counterDataListeners) {
    			counterDataListener.update("linesSkipped", getStepname(), linesSkipped+1);
			}
    	}
      return ++linesSkipped;
    }
   ...
   */
  public void addCounterDataListener(CounterDataListener listener) {
	  if(counterDataListeners == null) {
		  counterDataListeners = new ArrayList<CounterDataListener>();
	  }
	  counterDataListeners.add(listener);
  }
  
  /**
   * 增加 putRow 方法 和 getRow 方法 的监听器<br>
   * 当 isPut 返回 true ,putRow 方法才会调用<br>
   * 当 isGet 返回 true ,getRow 方法才会返回数据,否则继续等待<br>
   * @param listener 监听器对象
   */
  public void addWaitDataListener(WaitDataListener listener,boolean isGet) {
	  if(isGet) {
		  waitGetDataListener = listener;
	  }else {
		  waitPutDataListener = listener;
	  }
  ...
  private void specialPartitioning( RowMetaInterface rowMeta, Object[] row ) throws KettleStepException {
    ...
    if ( clusteredPartitioning ) {
      ...
      if ( selectedRowSet == null ) {
        ...
        incrementLinesWritten(selectedRowSet);
  ...
  private void putRowToRowSet( RowSet rs, RowMetaInterface rowMeta, Object[] row )  {
	if(waitPutDataListener != null &&  !waitPutDataListener.isPut(rs ,rowMeta, row)) {
		return ;
	}
  ...
  public void putRowTo( RowMetaInterface rowMeta, Object[] row, RowSet rowSet ) throws KettleStepException {
	  if(waitPutDataListener != null &&  !waitPutDataListener.isPut(rowSet ,rowMeta, row)) {
			 incrementLinesWritten(rowSet);
			 return ;
	  }
  ...
  public Object[] getRow() throws KettleException {
    Object[] d = getRowHandler().getRow();
    if(waitGetDataListener != null) {
    	while(d!= null && inputRowSets.size() > currentInputRowSetNr && !waitGetDataListener.isGet(currentInputStream() ,d)) {
    		d = getRowHandler().getRow();
    	}
	  }
    return d;
  }
  ...
  public Object[] getRowFrom( RowSet rowSet ) throws KettleStepException {
	  Object[] d = getRowHandler().getRowFrom( rowSet );
	  if(waitGetDataListener != null) {
	    	while(d!= null && !waitGetDataListener.isGet(rowSet ,d)) {
	    		d = getRowHandler().getRowFrom( rowSet );
	    	}
	  }
    return d;
  }
  ...
  public interface WaitDataListener{
	  /**
	   * 数据是否 put, 为 true时,调用 putRow 方法才会生效
	   * @param rowMeta
	   * @param row
	   * @return
	   * @throws KettleStepException
	   */
	  default public boolean isPut(RowSet rowSet ,RowMetaInterface rowMeta, Object[] row)  {
		  return true ;
	  }
	  /**
	   * 数据是否get ,为 true时,调用getRow方法才会返回,否则等待下一条数据
	   * @param row
	   * @return
	   * @throws KettleStepException
	   */
	  default public boolean isGet(RowSet rowSet , Object[] row)  {
		  return true ;
	  }
	   ...
	   * @throws KettleStepException
	   */
	  default public boolean isEnd(RowSet rowSet)  {
		  return true ;
	  }
  }
  ...
  public interface CounterDataListener{
	  ...
	  /**
	   * 数据更新,当数据行变化 时 调用
	   * @param key  : linesOutput,linesRead,linesInput,linesWritten,linesRejected,linesUpdated
	   * @param linesInput
	   * @param linesOutput
	   * @param linesRead
	   * @param linesWritten
	   ...
	   * @param linesErrors
	   */
	  default public void update(String key ,String stepName , long lines) {
