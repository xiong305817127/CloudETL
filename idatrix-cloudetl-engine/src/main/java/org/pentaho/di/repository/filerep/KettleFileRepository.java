...
public class KettleFileRepository extends AbstractRepository {
  ...
  public BaseMetaStore metaStore;
  ...
  public void connect( String username, String password ) throws KettleException {
		...
		directory.append(Const.NVL(username, MetaStoreConst.META_FOLDER));
		String metaStoreRootFolder = directory.toString();
		log.logBasic("KettleFileRepository.connect... " + metaStoreRootFolder);
		//xionghan  优先 VFS metastore
    	this.metaStore = (BaseMetaStore)OsgiBundleUtils.invokeOsgiMethod(Utils.getPackageName("com.ys.idatrix.cloudetl.repository.xml.metastore.vfsxml.VFSXmlMetaStore"), "createMetaStore", metaStoreRootFolder,true);
    	if(this.metaStore == null ) {
    		try {
    			File metaStoreRootFolderFile = new File(metaStoreRootFolder);
  ...
  //xionghan 增加根路径缓存
  public static volatile RepositoryDirectoryInterface rootDirectory ;

  @Override
  public RepositoryDirectoryInterface loadRepositoryDirectoryTree() throws KettleException {
	  if(rootDirectory ==  null ) {
		  synchronized (KettleFileRepository.class) {
			  if(rootDirectory ==  null ) {
				  RepositoryDirectory root = new RepositoryDirectory();
				  root.setObjectId( new StringObjectId( "/" ) );
				  rootDirectory = loadRepositoryDirectoryTree( root );
			  }
		}
  ...
  public void setMetaStore( BaseMetaStore metaStore ) {
