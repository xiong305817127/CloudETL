...
public class SFTPClient {
  ...
  //xionghan  增加获取子目录文件的方法
  public String[] dir(boolean isIncloudFloder) throws KettleJobException {
		      ...
		      if ( v != null ) {
		        for ( int i = 0; i < v.size(); i++ ) {
		          ...
		          if ( obj != null && obj instanceof com.jcraft.jsch.ChannelSftp.LsEntry ) {
		            ...
		            }else if(isIncloudFloder) {
		            	 if(lse.getFilename().matches("^(\\.|/)*$")) {
		                 	//排除  . | .. | / 等特殊文件名
		                 	continue ;
		                 }
			             String curPath = pwd();
			             getDirChildFile(o, lse.getFilename(), curPath+"/"+lse.getFilename());
			             chdir( curPath );
		            }
		          }
		        }
  ...
  private void getDirChildFile(java.util.Vector<String> o ,String  relateParentPath,String absolutParentPath ) throws KettleJobException, SftpException {

	  chdir( absolutParentPath );
      ...
      if ( v != null ) {
        for ( int i = 0; i < v.size(); i++ ) {
          ...
          if ( obj != null && obj instanceof com.jcraft.jsch.ChannelSftp.LsEntry ) {
            ...
            if ( !lse.getAttrs().isDir() ) {
              o.add( relateParentPath+"/"+lse.getFilename() );
            }else {
            	getDirChildFile(o, relateParentPath+"/"+lse.getFilename(),absolutParentPath+"/"+lse.getFilename());
            }
          }
        }
      }
...
//    }
//  }
 //xionghan  修复 mkdir多级目录创建失败的问题 
  public void createFolder(String foldername) throws KettleJobException {
	  try {
		   String pathArry[] = foldername.split("/");
		   StringBuffer filePath = new StringBuffer(foldername.charAt(0)=='/'?"/":pwd());
		   ...
		   for (String path : pathArry) {
			   ...
			   if (!folderExists(filePath.toString())) {
				   // 建立目录
				   c.mkdir(filePath.toString());
			   }
		   }
	  } catch (SftpException e) {
		  throw new KettleJobException( e );
