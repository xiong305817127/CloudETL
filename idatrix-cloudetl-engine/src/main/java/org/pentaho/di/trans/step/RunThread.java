...
public class RunThread implements Runnable {
	...
	public static final Log  logger = LogFactory.getLog("RunThread");
  ...
  public void run() {
    try {
      ...
      logger.error("转换["+step.getTrans().getName()+"]执行步骤["+step.getStepname()+"]发生错误:",t);
      //xionghan 当step运行时抛出了异常时,保存4W异常信息
  	  if( IdatrixPropertyUtil.getBooleanProperty("idatrix.exec.save.run.exception", true) || "true".equalsIgnoreCase( step.getVariable("idatrix.exec.save.run.exception"))) {
  		step.getTrans().save4WException(null, null,  step.getStepname(),  t ,null);
  	  }
      try {
        // check for OOME
        ...
        e.printStackTrace();
        log.logError("OutOfMemoryError:"+e.getMessage());
      } finally {
        step.setErrors( 1 );
        step.stopAll();
      }
