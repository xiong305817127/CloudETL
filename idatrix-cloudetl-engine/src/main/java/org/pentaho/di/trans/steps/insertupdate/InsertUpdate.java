...
public class InsertUpdate extends BaseStep implements StepInterface {
  ...
  protected synchronized void lookupValues( RowMetaInterface rowMeta, Object[] row ) throws KettleException {
    ...
    if ( add == null ) {
      ...
      //
      //xionghan  增加插入更新标志域和时间信息
      int newLen =0;
      newLen = (!Utils.isEmpty( meta.getSyncFlagFieldName())) ? (newLen+1):newLen;
      newLen = (!Utils.isEmpty( meta.getSyncTimeFieldName())) ? (newLen+1):newLen;
      Object[] insertRow = new Object[data.valuenrs.length+newLen];
      int i = 0 ;
      for ( ; i < data.valuenrs.length; i++ ) {
        insertRow[i] = row[data.valuenrs[i]];
      }

      //xionghan  增加插入更新标志域和时间信息
  	  if( !Utils.isEmpty( meta.getSyncFlagFieldName())) {
  		insertRow[i] = "I";
  		i++;
  	  }
  	  if( !Utils.isEmpty( meta.getSyncTimeFieldName()) ){
  		insertRow[i] = new Date();
  		 i++;
  	  }
      ...
      if ( !meta.isUpdateBypassed() ) {
        ...
        if ( update ) {
          ...
          newLen = (!Utils.isEmpty( meta.getSyncFlagFieldName())) ? (newLen+1):newLen;
          newLen = (!Utils.isEmpty( meta.getSyncTimeFieldName())) ? (newLen+1):newLen;
          Object[] updateRow = new Object[data.updateParameterRowMeta.size()+newLen];
          int j = 0;
          for ( int i = 0; i < data.valuenrs.length; i++ ) {
            if ( meta.getUpdate()[i].booleanValue() ) {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    if ( first ) {
      ...
      //xionghan  增加插入更新标志域和时间信息
      if( !Utils.isEmpty( meta.getSyncFlagFieldName()) ){
    	  data.insertRowMeta.addValueMeta(  new ValueMetaString(meta.getSyncFlagFieldName()) );
      }
      if( !Utils.isEmpty( meta.getSyncTimeFieldName()) ){
    	  data.insertRowMeta.addValueMeta(  new ValueMetaDate(meta.getSyncTimeFieldName()) );
      }
  ...
  public void prepareUpdate( RowMetaInterface rowMeta ) throws KettleDatabaseException {
    ...
    if( !Utils.isEmpty( meta.getSyncFlagFieldName()) ){
    	 if ( comma ) {
           ...
           sql += databaseMeta.quoteField( meta.getSyncFlagFieldName() );
           sql += " = ?" + Const.CR;
           data.updateParameterRowMeta.addValueMeta( new ValueMetaString(meta.getSyncFlagFieldName()) );
    }
    if( !Utils.isEmpty( meta.getSyncTimeFieldName()) ){
    	 if ( comma ) {
             sql += ",   ";
           ...
           sql += databaseMeta.quoteField( meta.getSyncTimeFieldName() );
           sql += " = ?" + Const.CR;
           data.updateParameterRowMeta.addValueMeta( new ValueMetaDate(meta.getSyncTimeFieldName()) );
    }
