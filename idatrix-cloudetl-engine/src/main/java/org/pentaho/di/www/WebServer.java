...
public class WebServer {
  ...
  public void startServer() throws Exception {
    ...
    //xionghan
    if (IdatrixPropertyUtil.getBooleanProperty("idatrix.security.deployment")) {
    ...
    HandlerList handlers = new HandlerList();
		if("true".equals(IdatrixPropertyUtil.getProperty("idatrix.web.deployment"))){
			log.logBasic("Adding cloud web module...");

			// Add new Servlet: StartDemoServlet, for Kettle SDK Demo
			// ServletContextHandler demoHandler =
			// new ServletContextHandler(contexts, StartDemoServlet.CONTEXT_PATH,
			// ServletContextHandler.SESSIONS);
			// demoHandler.addServlet(new ServletHolder(new
			// StartDemoServlet(jobMap)), "/*");

			// Set web context
			WebAppContext webContext = new WebAppContext();
			webContext.setDescriptor("./web/WEB-INF/web.xml");
			log.logBasic("Web Context Descriptor: " + webContext.getDescriptor());

			// Set application resource base path
			webContext.setResourceBase("./web");
			log.logBasic("Web Context Resource Base: " + webContext.getResourceBase());

			// Set context path (root)
			webContext.setContextPath(IdatrixPropertyUtil.getProperty("idatrix.web.contextPath" , "/cloudetl") );
			webContext.setClassLoader(Thread.currentThread().getContextClassLoader());
			webContext.setParentLoaderPriority(true);
			
			//cors filter 
			webContext.addFilter(Utils.getPackageName("com.ys.idatrix.cloudetl.web.filter.CloudCorsFilter"),  "/*", EnumSet.allOf(DispatcherType.class)) ;
			
			// Adding SSO security filter
			if ("true".equals(IdatrixPropertyUtil.getProperty("idatrix.security.deployment"))) {
				ServletHandler handler = new ServletHandler();
				FilterHolder filterHolder = handler.addFilterWithMapping("com.idatrix.unisecurity.sso.client.SSOFilter", "/*", EnumSet.allOf(DispatcherType.class));
				
				filterHolder.setName(IdatrixPropertyUtil.getProperty("idatrix.security.name"));
				filterHolder.setInitParameter("serverBaseUrl", IdatrixPropertyUtil.getProperty("idatrix.security.serverBaseUrl"));
				filterHolder.setInitParameter("serverInnerAddress", IdatrixPropertyUtil.getProperty("idatrix.security.serverInnerAddress"));
				filterHolder.setInitParameter("projectName", IdatrixPropertyUtil.getProperty("idatrix.security.projectName"));
				filterHolder.setInitParameter("notLoginOnFail", IdatrixPropertyUtil.getProperty("idatrix.security.notLoginOnFail"));
				filterHolder.setInitParameter("excludes",  IdatrixPropertyUtil.getProperty("idatrix.security.excludes",".*/(api-docs|webservice|password).*"));

				webContext.addFilter(filterHolder, "/*", EnumSet.allOf(DispatcherType.class));
			}
			
			//add  Global Filter,after sso
			webContext.addFilter(Utils.getPackageName("com.ys.idatrix.cloudetl.web.filter.GlobalFilter"), "/*", EnumSet.allOf(DispatcherType.class));

			handlers.setHandlers( new Handler[] { webContext, contexts, resourceHandler } );

		}else{
			
			//org.springframework.context.support.ClassPathXmlApplicationContext context = new org.springframework.context.support.ClassPathXmlApplicationContext(new String[] { "classpath:applicationContext.xml" });  
			springContext = OsgiBundleUtils.newOsgiInstance(Class.forName("org.springframework.context.support.ClassPathXmlApplicationContext"), null, "file:./web/WEB-INF/idatrix-application.xml" );
			//context.start(); 
			OsgiBundleUtils.invokeOsgiMethod(springContext, "start");
