...
public class HadoopSnappyCompressionProvider implements CompressionProvider {
  ...
  public static Object getActiveSnappyShim() throws Exception {
    ...
    if ( hadoopConfigUtilClassName != null ) {
      ...
      if ( hadoopConfigUtilClass == null ) {
    	 Object obj = getHadoopSnappyShimFromPlugin();
    	 if(obj !=null){
    		 return obj ;
    	 }
  ...
  //xionghan 从插件中获取snappyShim 对象
  private static Object getHadoopSnappyShimFromPlugin() throws Exception{
	  try{
		  PluginRegistry registry = PluginRegistry.getInstance() ;
	      PluginInterface pi = registry.findPluginWithId( KettleLifecyclePluginType.class, "HadoopConfigurationBootstrap" );
//	       ClassLoader classLoader = pi.getClass().getClassLoader();
	      Object hadoopConfigurationBootstrap = PluginRegistry.getInstance().loadClass(pi);
	      hadoopConfigurationBootstrap = OsgiBundleUtils.invokeOsgiMethod(hadoopConfigurationBootstrap,"getInstance");
	      Object hadoopConfigurationProvider = OsgiBundleUtils.invokeOsgiMethod(hadoopConfigurationBootstrap, "getProvider");
	      Object hadoopConfiguration = OsgiBundleUtils.invokeOsgiMethod(hadoopConfigurationProvider, GET_ACTIVE_CONFIGURATION_METHOD);
	      return  OsgiBundleUtils.invokeOsgiMethod(hadoopConfiguration, GET_SNAPPY_SHIM );
	  }catch(Exception e){
		  throw new Exception( "No Hadoop Configuration Utilities class found, unable to get active Snappy shim" );
	  }
