...
public class CloudServiceAspect {
	...
	/**
	 * 服务方法切点（包括com.ys.idatrix.quality.service.*的全部包， 但不包括*中的子包）
	 */
	@Pointcut("execution(* com.ys.idatrix.quality.service.*.*.*(..))"
			+ " && !@within(org.springframework.web.bind.annotation.ControllerAdvice)")
	private void servicePointcut2() {}
	
	/**
	 * 元数据处理方法切点（包括com.ys.idatrix.quality.metacube的全部包，包括子包）
	 */
	@Pointcut("execution(* com.ys.idatrix.quality.metacube..*.*(..))"
			+ " && !@within(org.springframework.web.bind.annotation.ControllerAdvice)")
...
//		CloudLogger.debug("服务结束: " + (returnVal != null ? CloudLogUtils.jsonLog2(returnVal) : "<NULL>"));
	}
