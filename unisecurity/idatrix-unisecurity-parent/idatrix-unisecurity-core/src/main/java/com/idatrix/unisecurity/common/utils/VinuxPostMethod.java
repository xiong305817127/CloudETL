...
public class VinuxPostMethod extends PostMethod {
    ...
    public Object executeMethod(String... type) {
        try {
            ...
            for(Map.Entry<String,String> entry:head.entrySet()){
                if (SecurityStringUtils.isNotBlank(entry.getKey(), entry.getValue())) {
            ...
            if (SecurityStringUtils.indexOf(JSONObject, type) > 0) {
                return net.sf.json.JSONObject.fromObject(result);
            }

            if (SecurityStringUtils.indexOf(JSONArray, type) > 0) {
    ...
    public void setJSONParameter(JSONObject parameter) {
        for (Iterator<?> iter = parameter.keys(); iter.hasNext(); ) {
            ...
            String value = null == str ? "" : SecurityStringUtils.trimToEmpty(str
                    .toString());
    ...
    public void setParameter(Map<String, Object> parameter) {
        for(Map.Entry<String,Object> entry:parameter.entrySet()){
             String value=entry.getValue()==null?"":SecurityStringUtils.trimToEmpty(entry.getValue().toString());
    ...
    public JsonConfig getConfig() {
        ...
        PropertyFilter pf = new PropertyFilter() {
            ...
            public boolean apply(Object source, String name, Object value) {
                return SecurityStringUtils.isBlank(value);
            }
        };
        // 将过滤器放入json-config中
